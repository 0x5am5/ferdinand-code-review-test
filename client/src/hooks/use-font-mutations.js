...
export function useFontMutations(clientId) {
    ...
    const addFont = useMutation({
        ...
        onError: (error) => {
            console.error("Font addition failed:", error instanceof Error ? error.message : "Unknown error");
            toast({
    ...
    const deleteFont = useMutation({
        mutationFn: async (fontId) => {
            ...
            const response = await apiRequest("DELETE", `/api/clients/${clientId}/assets/${fontId}`);
            return response.json();
        },
        onSuccess: () => {
            if (clientId) {
