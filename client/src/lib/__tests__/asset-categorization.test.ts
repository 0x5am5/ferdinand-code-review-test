...
import {
  ...
  getSupportedExtensionsForCategory,
  isSupportedFileType,
} from "../asset-categorization";
...
describe("Asset Categorization", () => {
  ...
  describe("findCategoryIdByName", () => {
    ...
    it("should handle case insensitive matching", () => {
      expect(
        findCategoryIdByName("documents" as AssetCategory, mockCategories)
      ).toBe(1);
      expect(
        findCategoryIdByName("PHOTOGRAPHY" as AssetCategory, mockCategories)
      ).toBe(5);
    });
    ...
    it("should return null for unknown category", () => {
      expect(
        findCategoryIdByName(
          "Unknown Category" as AssetCategory,
          mockCategories
        )
      ).toBe(null);
    });
