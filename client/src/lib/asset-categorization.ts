...
export function determineAssetCategory(file: File): AssetCategory | null {
  ...
  if (
    mimeType &&
    FILE_TYPE_CATEGORIES[mimeType as keyof typeof FILE_TYPE_CATEGORIES]
  ) {
 ...
 */
export function getSupportedExtensionsForCategory(
 ...
 */
export function getCategorizationReason(file: File): string {
  const mimeType = file.type.toLowerCase();
  const fileName = file.name.toLowerCase();
  const extension = fileName.substring(fileName.lastIndexOf("."));
...
export function isValidCategory(
  ...
  return validCategories.includes(categoryName as AssetCategory);
}
