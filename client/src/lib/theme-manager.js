...
class ThemeManager {
        ...
        try {
            ...
            if (newTheme.theme) {
                ...
                if (newTheme.theme.variant || newTheme.theme.primary) {
                    const variantColors = this.generateVariantColors(newTheme.theme.primary || this.draftTheme.theme.primary, newTheme.theme.variant || this.draftTheme.theme.variant);
    ...
    applyTheme() {
        ...
        this.applyThemeTimeout = setTimeout(() => {
            ...
            if (snapshot === this.lastAppliedTheme)
                return;
            ...
            try {
                ...
                if (activeTheme.theme.primary) {
                    ...
                    for (let i = 0; i <= 100; i += 10) {
                        const neutralKey = `neutral-${i}`;
                        if (activeTheme.colors[neutralKey]) {
                            this.setCssColorProperty(`--${neutralKey}`, activeTheme.colors[neutralKey]);
                        }
                    }
                    ...
                    if (activeTheme.theme.appearance === "dark") {
                        ...
                        if (activeTheme.colors["primary-foreground"]) {
                            this.setCssColorProperty("--primary-foreground", activeTheme.colors["primary-foreground"]);
                        }
                    }
                }
                // Apply typography
    ...
    generateVariantColors(primaryColor, variant) {
    ...
    setCssColorProperty(property, value) {
        const root = document.documentElement;
        try {
            if (value.startsWith("#")) {
...
export function useThemeManager() {
    return {
        ...
        updateDraftTheme: (theme) => themeManager.updateDraftTheme(theme),
