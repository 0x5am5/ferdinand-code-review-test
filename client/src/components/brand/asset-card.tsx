...
export function AssetCard({ asset }: AssetCardProps) {
  ...
  const renderLogoAsset = () => {
    const data = asset.data as { type: string; formats: Array<{ format: string; url: string }> };
    return (
      <>
        ...
        <div className="mt-4 space-y-2">
          ...
          <Button variant="secondary" size="sm" asChild className="w-full">
            <a href={data.formats[selectedFormat].url} download={`${asset.name}.${data.formats[selectedFormat].format}`}>
  ...
  const renderColorAsset = () => {
    const data = asset.data as { hex: string; labels: { rgb?: string; cmyk?: string; pantone?: string } };
    return (
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {Object.entries(data.labels).map(([format, value]) => (
            value && (
              <Button 
                key={format}
                variant="outline" 
        ...
        <div className="space-y-2">
          {data.url && (
            <Button variant="secondary" size="sm" asChild className="w-full">
              ...
              >
                {data.source === "google" ? "View on Google Fonts" : 
                 data.source === "adobe" ? "View on Adobe Fonts" : 
                 "Download Font"}
              </a>
            </Button>
          )}
