...
export const ThemeProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const _generateContainerVariant = (hex: string) => {
    if (!isHexColor(hex)) return hex;
  ...
  const applyThemeToDom = useCallback(() => {
    ...
    try {
      ...
      if (activeSystem.theme.primary) {
        ...
        const _primaryTint1 = generateTint(activeSystem.theme.primary, 20);
        const _primaryTint2 = generateTint(activeSystem.theme.primary, 40);
        const _primaryShade1 = generateShade(activeSystem.theme.primary, 20);
        const _primaryShade2 = generateShade(activeSystem.theme.primary, 40);
