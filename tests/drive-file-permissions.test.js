...
import { hasPermission, canImportDriveFiles, checkDriveFilePermission, getInitialImportPermissions, getRolePermissionDescription, getRolePermissions, DRIVE_FILE_PERMISSIONS, } from '../server/services/drive-file-permissions';
describe('Drive File Permissions', () => {
    ...
    describe('hasPermission() - Base Permission Checks', () => {
        describe('GUEST role', () => {
            it('should allow read for shared files only', () => {
                expect(hasPermission(UserRole.GUEST, 'read', { assetVisibility: 'shared' })).toBe(true);
                expect(hasPermission(UserRole.GUEST, 'read', { assetVisibility: 'private' })).toBe(false);
            });
            it('should deny write, delete, and share permissions', () => {
                expect(hasPermission(UserRole.GUEST, 'write', { assetVisibility: 'shared' })).toBe(false);
                expect(hasPermission(UserRole.GUEST, 'delete', { assetVisibility: 'shared' })).toBe(false);
                expect(hasPermission(UserRole.GUEST, 'share', { assetVisibility: 'shared' })).toBe(false);
            });
        });
        describe('STANDARD role', () => {
            it('should allow read for all files', () => {
                expect(hasPermission(UserRole.STANDARD, 'read', { assetVisibility: 'shared' })).toBe(true);
                expect(hasPermission(UserRole.STANDARD, 'read', {
                    assetVisibility: 'private',
                })).toBe(true);
            });
            it('should allow write only for owned files', () => {
                expect(hasPermission(UserRole.STANDARD, 'write', { isOwner: true })).toBe(true);
                expect(hasPermission(UserRole.STANDARD, 'write', { isOwner: false })).toBe(false);
            });
            it('should deny delete regardless of ownership', () => {
                expect(hasPermission(UserRole.STANDARD, 'delete', { isOwner: true })).toBe(false);
                expect(hasPermission(UserRole.STANDARD, 'delete', { isOwner: false })).toBe(false);
            });
    ...
    describe('checkDriveFilePermission() - Comprehensive Permission Checks', () => {
        ...
        describe('STANDARD role checks', () => {
            it('should allow read for any file', () => {
                const result = checkDriveFilePermission(123, UserRole.STANDARD, 'read', mockAssetData);
                expect(result.allowed).toBe(true);
            });
            ...
            it('should deny write for files owned by others', () => {
                const result = checkDriveFilePermission(456, UserRole.STANDARD, 'write', {
                    ...mockAssetData,
            ...
            it('should deny delete permission', () => {
                const result = checkDriveFilePermission(123, UserRole.STANDARD, 'delete', mockAssetData);
                expect(result.allowed).toBe(false);
                expect(result.reason).toContain('does not have delete permission');
            });
        });
        describe('EDITOR role checks', () => {
            it('should allow read for any file', () => {
                const result = checkDriveFilePermission(456, UserRole.EDITOR, 'read', mockAssetData);
                expect(result.allowed).toBe(true);
            });
            it('should allow write for any file', () => {
                const result = checkDriveFilePermission(456, UserRole.EDITOR, 'write', mockAssetData);
                expect(result.allowed).toBe(true);
            });
            it('should allow share permission', () => {
                const result = checkDriveFilePermission(456, UserRole.EDITOR, 'share', mockAssetData);
                expect(result.allowed).toBe(true);
            });
            it('should deny delete permission', () => {
                const result = checkDriveFilePermission(456, UserRole.EDITOR, 'delete', mockAssetData);
                expect(result.allowed).toBe(false);
                expect(result.reason).toContain('does not have delete permission');
            });
        });
        describe('ADMIN role checks', () => {
            it('should allow all permissions', () => {
                expect(checkDriveFilePermission(789, UserRole.ADMIN, 'read', mockAssetData)
                    .allowed).toBe(true);
                expect(checkDriveFilePermission(789, UserRole.ADMIN, 'write', mockAssetData)
                    .allowed).toBe(true);
                expect(checkDriveFilePermission(789, UserRole.ADMIN, 'delete', mockAssetData)
                    .allowed).toBe(true);
                expect(checkDriveFilePermission(789, UserRole.ADMIN, 'share', mockAssetData)
                    .allowed).toBe(true);
            });
        });
        describe('SUPER_ADMIN role checks', () => {
            it('should allow all permissions', () => {
                expect(checkDriveFilePermission(789, UserRole.SUPER_ADMIN, 'read', mockAssetData).allowed).toBe(true);
                expect(checkDriveFilePermission(789, UserRole.SUPER_ADMIN, 'write', mockAssetData).allowed).toBe(true);
                expect(checkDriveFilePermission(789, UserRole.SUPER_ADMIN, 'delete', mockAssetData).allowed).toBe(true);
                expect(checkDriveFilePermission(789, UserRole.SUPER_ADMIN, 'share', mockAssetData).allowed).toBe(true);
            });
        });
    });
    describe('getInitialImportPermissions() - Import Settings', () => {
        ...
        it('should set importing user as Ferdinand owner', () => {
            const result = getInitialImportPermissions(123, UserRole.STANDARD, mockDriveMetadata);
