 ...
 */
export async function createTestUser(email, role, name) {
 ...
 */
export async function createTestClient(name) {
 ...
 */
export async function associateUserWithClient(userId, clientId) {
...
export async function cleanupTestClient(clientId) {
    // Delete user-client associations
 ...
 */
export async function cleanupTestAssets(assetIds) {
    if (assetIds.length === 0)
        return;
 ...
 */
export async function cleanupTestCategories(categoryIds) {
    if (categoryIds.length === 0)
        return;
    const { assetCategories, assetCategoryAssignments } = await import('@shared/schema');
    // Delete category assignments first
    await db.delete(assetCategoryAssignments).where(inArray(assetCategoryAssignments.categoryId, categoryIds));
    // Delete categories
 ...
 * Clean up tags by ID
 */
export async function cleanupTestTags(tagIds) {
    if (tagIds.length === 0)
        return;
    const { assetTags, assetTagAssignments } = await import('@shared/schema');
    // Delete tag assignments first
    await db.delete(assetTagAssignments).where(inArray(assetTagAssignments.tagId, tagIds));
    // Delete tags
