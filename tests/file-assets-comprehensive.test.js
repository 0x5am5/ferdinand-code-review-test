...
import { createTestUser, createTestClient, associateUserWithClient, cleanupTestUser, cleanupTestClient, cleanupTestAssets, cleanupTestCategories, cleanupTestTags, } from './helpers/test-server';
...
const createdTagIds = [];
// Helper functions
function createTestFile(filename, sizeKB = 10) {
    const content = Buffer.alloc(sizeKB * 1024, 'test data ');
    return content;
}
async function authenticatedFetch(url, userType, options = {}) {
 ...
 */
async function setupTestUser(userType, role) {
...
describe('File Asset System - Comprehensive Tests', () => {
    ...
    async function setupTestAssets() {
        ...
        const sharedRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/upload`, 'admin', {
            method: 'POST',
            body: sharedFormData,
        });
        if (sharedRes.ok) {
        ...
        const privateRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/upload`, 'admin', {
            method: 'POST',
    ...
    describe('Role-Based Permissions', () => {
        describe('Guest User', () => {
            it('should only see shared assets', async () => {
                const response = await authenticatedFetch(`${API_BASE}/assets`, 'guest');
                expect(response.status).toBe(200);
                const assets = await response.json();
                expect(Array.isArray(assets)).toBe(true);
                // All returned assets should be shared
                const allShared = assets.every((a) => a.visibility === 'shared');
                expect(allShared).toBe(true);
            });
            it('should not access private assets', async () => {
                const response = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${privateAssetId}`, 'guest');
            ...
            it('should not be able to upload files', async () => {
                ...
                const response = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/upload`, 'guest', {
                    method: 'POST',
                    body: formData,
                });
                expect(response.status).toBe(403);
            ...
            it('should not be able to delete assets', async () => {
                const response = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${sharedAssetId}`, 'guest', {
                    method: 'DELETE',
                });
                expect(response.status).toBe(403);
            });
            it('should be able to download shared assets', async () => {
                const response = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${sharedAssetId}/download`, 'guest');
                expect(response.status).toBe(200);
        ...
        describe('Standard User', () => {
            it('should see all shared assets and own private assets', async () => {
                const response = await authenticatedFetch(`${API_BASE}/assets`, 'standard');
                expect(response.status).toBe(200);
                const assets = await response.json();
                expect(Array.isArray(assets)).toBe(true);
            });
            it('should be able to upload files', async () => {
                ...
                const response = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/upload`, 'standard', {
                    method: 'POST',
                    body: formData,
                });
            ...
            it('should be able to update own assets', async () => {
                ...
                const uploadRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/upload`, 'standard', {
                    method: 'POST',
                    body: formData,
                });
                ...
                trackAsset(asset.id);
                // Now update it
                const updateRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${asset.id}`, 'standard', {
                    method: 'PATCH',
            ...
            it('should not be able to delete other users assets', async () => {
                const response = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${sharedAssetId}`, 'standard', {
                    method: 'DELETE',
                });
            ...
            it('should be able to delete own assets', async () => {
                ...
                trackAsset(asset.id);
                // Now delete it
                const deleteRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${asset.id}`, 'standard', {
                    method: 'DELETE',
                });
        ...
        describe('Editor User', () => {
            it('should be able to edit shared assets', async () => {
                const response = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${sharedAssetId}`, 'editor', {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ visibility: 'shared' }),
                });
            ...
            it('should be able to upload files', async () => {
                ...
                const response = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/upload`, 'editor', {
                    method: 'POST',
                    body: formData,
                });
                expect(response.status).toBe(201);
        ...
        describe('Admin User', () => {
            it('should have full CRUD access to all assets', async () => {
                ...
                const createRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/upload`, 'admin', {
                    method: 'POST',
                    body: formData,
                });
                ...
                trackAsset(asset.id);
                // Read
                const readRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${asset.id}`, 'admin');
                expect(readRes.status).toBe(200);
                // Update
                const updateRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${asset.id}`, 'admin', {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ visibility: 'shared' }),
                });
                expect(updateRes.status).toBe(200);
                // Delete
                const deleteRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${asset.id}`, 'admin', {
                    method: 'DELETE',
                });
                expect(deleteRes.status).toBe(200);
            });
            ...
            it('should be able to manage categories', async () => {
                // Create category
                const createRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-asset-categories`, 'admin', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                ...
                // Update category
                const updateRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-asset-categories/${testCategoryId}`, 'admin', {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
    ...
    describe('Search Functionality', () => {
        beforeAll(async () => {
            ...
            for (const asset of testAssets) {
                for (const tagName of asset.tags) {
                    if (!tagMap[tagName]) {
                        const tagRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-asset-tags`, 'admin', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                                name: tagName,
            ...
            for (const asset of testAssets) {
                ...
                formData.append('tagIds', JSON.stringify(asset.tags.map((t) => tagMap[t])));
                const response = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/upload`, 'admin', {
                    method: 'POST',
                    body: formData,
                });
                if (response.ok) {
        ...
        it('should search by filename', async () => {
            const response = await authenticatedFetch(`${API_BASE}/assets?search=marketing`, 'admin');
            expect(response.status).toBe(200);
            const assets = await response.json();
            expect(assets.length).toBeGreaterThan(0);
            expect(assets.some((a) => a.fileName.includes('marketing'))).toBe(true);
        });
        it('should search by tags', async () => {
            const response = await authenticatedFetch(`${API_BASE}/assets?search=photography`, 'admin');
            expect(response.status).toBe(200);
            const assets = await response.json();
            expect(assets.length).toBeGreaterThan(0);
        });
        it('should use dedicated search endpoint', async () => {
            const response = await authenticatedFetch(`${API_BASE}/assets/search?q=financial`, 'admin');
            expect(response.status).toBe(200);
        ...
        it('should return empty results for non-matching search', async () => {
            const response = await authenticatedFetch(`${API_BASE}/assets/search?q=nonexistentfile12345`, 'admin');
            expect(response.status).toBe(200);
            const data = await response.json();
        ...
        it('should respect role permissions in search results', async () => {
            ...
            const response = await authenticatedFetch(`${API_BASE}/assets/search?q=brochure`, 'guest');
            expect(response.status).toBe(200);
            const data = await response.json();
            // All results should be shared assets
            if (data.results.length > 0) {
                const allShared = data.results.every((a) => a.visibility === 'shared');
                expect(allShared).toBe(true);
            }
        });
        it('should combine search with filters', async () => {
            const response = await authenticatedFetch(`${API_BASE}/assets?search=q4&visibility=shared`, 'admin');
            expect(response.status).toBe(200);
            const assets = await response.json();
    ...
    describe('Thumbnail Generation', () => {
        ...
        beforeAll(async () => {
            ...
            const imageRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/upload`, 'admin', {
                method: 'POST',
                body: imageFormData,
            });
            if (imageRes.ok) {
            ...
            const pdfRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/upload`, 'admin', {
                method: 'POST',
                body: pdfFormData,
            });
            if (pdfRes.ok) {
        ...
        it('should generate thumbnail for image files', async () => {
            ...
            for (const size of sizes) {
                const response = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${imageAssetId}/thumbnail/${size}`, 'admin');
                expect(response.status).toBe(200);
        ...
        it('should reject invalid thumbnail sizes', async () => {
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${imageAssetId}/thumbnail/invalid`, 'admin');
            expect(response.status).toBe(400);
        });
        it('should return file type icon for non-previewable files', async () => {
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${pdfAssetId}/thumbnail/small`, 'admin');
        ...
        it('should cache generated thumbnails', async () => {
            ...
            const response1 = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${imageAssetId}/thumbnail/small`, 'admin');
            const time1 = Date.now() - start1;
            expect(response1.status).toBe(200);
            // Second request - should use cached version
            const start2 = Date.now();
            const response2 = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${imageAssetId}/thumbnail/small`, 'admin');
        ...
        it('should enforce permissions on thumbnail access', async () => {
            ...
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${privateAssetId}/thumbnail/small`, 'guest');
            expect(response.status).toBe(403);
        });
        it('should delete thumbnails when asset is deleted', async () => {
            ...
            const uploadRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/upload`, 'admin', {
                method: 'POST',
                body: formData,
            });
            const asset = await uploadRes.json();
            trackAsset(asset.id);
            // Generate thumbnail
            await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${asset.id}/thumbnail/small`, 'admin');
            // Delete asset
            const deleteRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${asset.id}`, 'admin', {
                method: 'DELETE',
            });
            ...
            const thumbnailRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${asset.id}/thumbnail/small`, 'admin');
    ...
    describe('Integration Scenarios', () => {
        it('should handle multi-file upload workflow', async () => {
            ...
            const listRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets`, 'admin');
            const data = await listRes.json();
            const uploadedIds = uploadedAssets.map((a) => a.id);
            const foundAssets = data.assets.filter((a) => uploadedIds.includes(a.id));
            expect(foundAssets.length).toBe(uploadedAssets.length);
        });
        it('should handle asset organization workflow', async () => {
            // 1. Create a category
            const categoryRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-asset-categories`, 'admin', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            const getRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${asset.id}`, 'admin');
        ...
        it('should handle permission escalation workflow', async () => {
            ...
            const guestRes = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${asset.id}`, 'guest');
            ...
            const guestRes2 = await authenticatedFetch(`${API_BASE}/clients/${testClient.id}/file-assets/${asset.id}`, 'guest');
    ...
    describe('Bulk Delete', () => {
        it('should bulk delete multiple assets with proper permissions', async () => {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                formData.append('visibility', 'shared');
                const uploadRes = await authenticatedFetch(`${API_BASE}/assets/upload`, 'editor', {
                    method: 'POST',
                    body: formData,
                });
            ...
            const bulkDeleteRes = await authenticatedFetch(`${API_BASE}/assets/bulk-delete`, 'editor', {
                method: 'POST',
                headers: {
            ...
            for (const assetId of assetIds) {
                const getRes = await authenticatedFetch(`${API_BASE}/assets/${assetId}`, 'editor');
        ...
        it('should reject bulk delete without proper permissions', async () => {
            ...
            const bulkDeleteRes = await authenticatedFetch(`${API_BASE}/assets/bulk-delete`, 'guest', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
        ...
        it('should handle bulk delete with non-existent asset IDs', async () => {
            ...
            const bulkDeleteRes = await authenticatedFetch(`${API_BASE}/assets/bulk-delete`, 'admin', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
        ...
        it('should only delete accessible assets in bulk operation', async () => {
            ...
            const uploadRes1 = await authenticatedFetch(`${API_BASE}/assets/upload`, 'editor', {
                method: 'POST',
                body: formData1,
            });
            ...
            const getRes = await authenticatedFetch(`${API_BASE}/assets/${asset1.id}`, 'editor');
        ...
        it('should delete files and thumbnails during bulk delete', async () => {
            ...
            formData.append('visibility', 'shared');
            const uploadRes = await authenticatedFetch(`${API_BASE}/assets/upload`, 'admin', {
                method: 'POST',
                body: formData,
            });
            const asset = await uploadRes.json();
            ...
            await authenticatedFetch(`${API_BASE}/assets/${asset.id}/thumbnail/small`, 'admin');
            ...
            const thumbnailRes = await authenticatedFetch(`${API_BASE}/assets/${asset.id}/thumbnail/small`, 'admin');
            expect(thumbnailRes.status).toBe(404);
        });
    });
    afterAll(async () => {
