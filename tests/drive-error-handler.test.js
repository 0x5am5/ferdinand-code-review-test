...
import { DriveErrorCode, createErrorResponse, handlePermissionDenied, handleDrivePermissionDenied, handleAuthError, handleFileNotFound, handleValidationError, handleDriveApiError, handleSystemError, isPermissionError, isNotFoundError, isAuthError, } from '../server/services/drive-error-handler';
...
describe('Drive Error Handler Service', () => {
    ...
    describe('createErrorResponse() - Error Response Builder', () => {
        ...
        it('should use custom message when provided', () => {
            const response = createErrorResponse(DriveErrorCode.PERMISSION_DENIED, 'Custom permission message');
            expect(response.message).toBe('Custom permission message');
            expect(response.code).toBe('PERMISSION_DENIED');
        });
        it('should include details when provided', () => {
            const response = createErrorResponse(DriveErrorCode.FILE_ACCESS_ERROR, undefined, 'Network timeout after 30 seconds');
            expect(response.details).toBe('Network timeout after 30 seconds');
        });
        it('should include metadata fields when provided', () => {
            const response = createErrorResponse(DriveErrorCode.ACTION_NOT_PERMITTED, undefined, undefined, {
                allowedAction: 'read',
        ...
        it('should not include undefined metadata fields', () => {
            const response = createErrorResponse(DriveErrorCode.PERMISSION_DENIED, undefined, undefined, {
    ...
    describe('Edge Cases and Security', () => {
        ...
        it('should not leak sensitive information in error messages', () => {
            const allMessages = Object.values(DriveErrorCode).map((code) => createErrorResponse(code).message.toLowerCase());
