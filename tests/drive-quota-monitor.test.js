...
import { trackQuotaUsage, trackQuotaError, getQuotaStats, resetQuotaTracking, withQuotaTracking, checkQuotaAlerts, getRecentQuotaErrors, cleanupExpiredQuotas, } from '../server/services/drive-quota-monitor';
 ...
 */
function createGaxiosError(statusCode, reason, message) {
    const error = {
        response: {
            status: statusCode,
...
describe('Drive Quota Monitor - trackQuotaError()', () => {
    ...
    it('should log error to console', () => {
        ...
        const parsedError = parseDriveError(error);
        trackQuotaError(parsedError, 'user123');
        expect(consoleErrorSpy).toHaveBeenCalledWith('Drive API quota error:', expect.objectContaining({
            userId: 'user123',
...
describe('Drive Quota Monitor - resetQuotaTracking()', () => {
    ...
    it('should log reset message', () => {
        ...
        expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Reset quota tracking for user user123'));
    });
});
// ============================================================================
...
describe('Drive Quota Monitor - withQuotaTracking()', () => {
    ...
    it('should throw error when user quota exceeded', async () => {
        ...
        const operation = jest.fn().mockResolvedValue('success');
        await expect(withQuotaTracking('user123', 'files.list', operation)).rejects.toThrow('Drive API quota exceeded for user user123');
    ...
    it('should throw error when global quota exceeded', async () => {
        ...
        const operation = jest.fn().mockResolvedValue('success');
        await expect(withQuotaTracking('user999', 'files.list', operation)).rejects.toThrow('Drive API quota exceeded');
        expect(operation).not.toHaveBeenCalled();
    });
    it('should warn at high quota usage', async () => {
        ...
        expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Drive API quota warning'), expect.objectContaining({
    ...
    it('should track quota errors on operation failure', async () => {
        ...
        await expect(withQuotaTracking('user123', 'files.list', operation)).rejects.toThrow();
...
describe('Drive Quota Monitor - cleanupExpiredQuotas()', () => {
    ...
    it('should log cleanup message when quotas cleaned', async () => {
        ...
        // Run cleanup
        cleanupExpiredQuotas();
        expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Cleaned up 1 expired quota windows'));
    });
});
