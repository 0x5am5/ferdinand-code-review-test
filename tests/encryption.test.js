...
import { encrypt, decrypt, encryptTokens, decryptTokens, isTokenExpired, } from '../server/utils/encryption';
describe('Token Encryption - AES-256-GCM', () => {
    ...
    describe('Security Properties', () => {
        ...
        test('should handle concurrent encryption operations', async () => {
            ...
            const results = await Promise.all(operations.map(async ({ original }) => {
                const encrypted = encrypt(original);
                const decrypted = decrypt(encrypted);
    ...
    describe('Integration - Token Lifecycle', () => {
        test('should simulate complete OAuth token lifecycle', () => {
            ...
            const { encryptedAccessToken, encryptedRefreshToken, expiresAt } = encryptTokens(oauthResponse);
