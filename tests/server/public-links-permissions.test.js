...
import { createTestUser, createTestClient, associateUserWithClient, cleanupTestUser, cleanupTestClient } from '../helpers/test-server.js';
describe('Public Links and Cross-Client Access Permissions', () => {
    let testSetup;
    beforeEach(async () => {
        testSetup = {};
        // Import schema dynamically like the service does
        const { users, clients, userClients, assets, UserRole } = await import('../../shared/schema.js');
    ...
    afterEach(async () => {
        ...
        if (testSetup && testSetup.sharedAsset) {
            ...
            await db.delete(assetPublicLinks).where(eq(assetPublicLinks.assetId, testSetup.sharedAsset.id));
            await db.delete(assetPublicLinks).where(eq(assetPublicLinks.assetId, testSetup.privateAsset.id));
            // Clean up assets
    ...
    describe('Public Link Creation Permissions', () => {
        it('should deny GUEST users from creating public links', async () => {
            ...
            const permission = await checkAssetPermission(guestUser.id, sharedAsset.id, clientA.id, 'share');
        ...
        it('should allow EDITOR users to create public links for shared assets', async () => {
            ...
            const permission = await checkAssetPermission(editorUser.id, sharedAsset.id, clientA.id, 'share');
        ...
        it('should allow EDITOR users to create public links for private assets they own', async () => {
            ...
            // Act: Check share permission for editor user
            const permission = await checkAssetPermission(editorUser.id, privateAsset.id, clientA.id, 'share');
            // Assert: Permission should be allowed
            expect(permission.allowed).toBe(true);
            expect(permission.asset).toBeDefined();
        ...
        it('should deny GUEST users from accessing private assets', async () => {
            ...
            const permission = await checkAssetPermission(guestUser.id, privateAsset.id, clientA.id, 'read');
            // Assert: Permission should be denied
            expect(permission.allowed).toBe(false);
        ...
        it('should allow GUEST users to access shared assets', async () => {
            ...
            const { guestUser, clientA, sharedAsset } = testSetup;
            // Act: Check read permission for guest user
            const permission = await checkAssetPermission(guestUser.id, sharedAsset.id, clientA.id, 'read');
            // Assert: Permission should be allowed
            expect(permission.allowed).toBe(true);
            expect(permission.asset).toBeDefined();
            expect(permission.asset?.id).toBe(sharedAsset.id);
    ...
    describe('Cross-Client Access Prevention', () => {
        it('should deny access to assets in other clients', async () => {
            ...
            const { guestUser, clientA, clientB, sharedAsset } = testSetup;
            // Act: Check permission with wrong client ID
            const permission = await checkAssetPermission(guestUser.id, sharedAsset.id, // This asset belongs to client A
        ...
        it('should deny users not associated with a client from accessing its assets', async () => {
            ...
            const permission = await checkAssetPermission(unassociatedUser.id, testSetup.sharedAsset.id, testSetup.clientA.id, 'read');
            // Assert: Permission should be denied
            expect(permission.allowed).toBe(false);
        ...
        it('should allow users to access assets in their associated clients', async () => {
            ...
            const permission = await checkAssetPermission(editorUser.id, sharedAsset.id, clientA.id, 'read');
            // Assert: Permission should be allowed
            expect(permission.allowed).toBe(true);
            expect(permission.asset).toBeDefined();
        });
        it('should prevent cross-client public link creation', async () => {
            ...
            const permission = await checkAssetPermission(testSetup.editorUser.id, clientBAsset.id, testSetup.clientA.id, // Wrong client ID
            'share');
            // Assert: Permission should be denied
            expect(permission.allowed).toBe(false);
            expect(permission.reason).toContain('Asset not in client');
        ...
        it('should enforce correct permission matrix for all roles', async () => {
            ...
            for (const role of roles) {
                ...
                for (const [permissionType, expected] of Object.entries(permissions)) {
                    const permission = await checkAssetPermission(testUser.id, sharedAsset.id, clientA.id, permissionType);
    ...
    describe('Edge Cases and Error Handling', () => {
        it('should handle non-existent users gracefully', async () => {
            ...
            const permission = await checkAssetPermission(nonExistentUserId, testSetup.sharedAsset.id, testSetup.clientA.id, 'read');
            // Assert: Permission should be denied
            expect(permission.allowed).toBe(false);
        ...
        it('should handle non-existent assets gracefully', async () => {
            ...
            const permission = await checkAssetPermission(testSetup.guestUser.id, nonExistentAssetId, testSetup.clientA.id, 'read');
            // Assert: Permission should be denied
            expect(permission.allowed).toBe(false);
