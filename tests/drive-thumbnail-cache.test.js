...
import { fetchAndCacheThumbnail, getCachedThumbnailPath, invalidateThumbnailCache, clearExpiredThumbnails, getThumbnailCacheStats, } from "../server/services/drive-thumbnail-cache";
...
describe("Drive Thumbnail Cache", () => {
    ...
    it("should fetch and cache a thumbnail", async () => {
        const result = await fetchAndCacheThumbnail(mockDriveClient, testAssetId, testDriveFileId, testModifiedDate, testThumbnailUrl, "medium");
    ...
    it("should return cached thumbnail on second fetch", async () => {
        // First fetch
        await fetchAndCacheThumbnail(mockDriveClient, testAssetId, testDriveFileId, testModifiedDate, testThumbnailUrl, "medium");
    ...
    it("should re-fetch when cache version changes", async () => {
        ...
        const result = await fetchAndCacheThumbnail(mockDriveClient, testAssetId, testDriveFileId, newModifiedDate, testThumbnailUrl, "medium");
    ...
    it("should handle different thumbnail sizes", async () => {
        ...
        for (const size of sizes) {
            const result = await fetchAndCacheThumbnail(mockDriveClient, testAssetId, testDriveFileId, testModifiedDate, testThumbnailUrl, size);
            expect(result).toBeDefined();
    ...
    it("should handle missing thumbnail gracefully", async () => {
        ...
        await expect(fetchAndCacheThumbnail(mockDriveClient, nonExistentAssetId, "non-existent-file", testModifiedDate, testThumbnailUrl, "medium")).rejects.toThrow("Asset not found");
    });
});
