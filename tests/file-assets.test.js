...
describe('File Asset System', () => {
    ...
    describe('Asset Upload', () => {
        it('should upload a file successfully', async () => {
            ...
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-assets/upload`, {
                method: 'POST',
                body: formData,
            });
        ...
        it('should upload file with categories and tags', async () => {
            ...
            const categoryRes = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-asset-categories`, {
                method: 'POST',
            ...
            const tagRes = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-asset-tags`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    name: 'Test Tag',
    ...
    describe('Asset Listing', () => {
        it('should list all assets for a client', async () => {
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-assets`);
        ...
        it('should support pagination', async () => {
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-assets?limit=2&offset=0`);
            expect(response.status).toBe(200);
            const data = await response.json();
        ...
        it('should filter by category', async () => {
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-assets?categoryId=${testCategoryId}`);
            expect(response.status).toBe(200);
            const data = await response.json();
            expect(data.assets.length).toBeGreaterThan(0);
        });
        it('should filter by tag', async () => {
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-assets?tagId=${testTagId}`);
            expect(response.status).toBe(200);
            const data = await response.json();
            expect(data.assets.length).toBeGreaterThan(0);
        });
        it('should filter by visibility', async () => {
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-assets?visibility=shared`);
            expect(response.status).toBe(200);
            const data = await response.json();
            expect(data.assets.every((a) => a.visibility === 'shared')).toBe(true);
        });
    });
    describe('Asset Retrieval', () => {
        it('should get a single asset by ID', async () => {
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-assets/${testAssetId}`);
            expect(response.status).toBe(200);
            const asset = await response.json();
        ...
        it('should return 404 for non-existent asset', async () => {
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-assets/999999`);
            expect(response.status).toBe(404);
        });
    });
    describe('Asset Download', () => {
        it('should download an asset file', async () => {
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-assets/${testAssetId}/download`);
            expect(response.status).toBe(200);
    ...
    describe('Asset Deletion', () => {
        it('should soft delete an asset', async () => {
            ...
            const getResponse = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-assets/${testAssetId}`);
            expect(getResponse.status).toBe(403);
        });
    });
    describe('Category Management', () => {
        it('should list all categories (system + client)', async () => {
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-asset-categories`);
            expect(response.status).toBe(200);
            const categories = await response.json();
            expect(Array.isArray(categories)).toBe(true);
            expect(categories.some((c) => c.isDefault)).toBe(true);
        });
        ...
        it('should update a category (admin only)', async () => {
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-asset-categories/${testCategoryId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
        ...
        it('should not allow editing system default categories', async () => {
            ...
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-asset-categories/1`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    name: 'Hacked Name',
    ...
    describe('Tag Management', () => {
        it('should list all tags for a client', async () => {
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-asset-tags`);
            expect(response.status).toBe(200);
        ...
        it('should delete a tag (admin only)', async () => {
            const response = await authenticatedFetch(`${API_BASE}/clients/${testClientId}/file-asset-tags/${testTagId}`, {
                method: 'DELETE',
            });
            expect([200, 403]).toContain(response.status);
        });
    ...
    describe('Permission System', () => {
        it('should enforce authentication on all endpoints', async () => {
            ...
            const response = await fetch(`${API_BASE}/clients/${testClientId}/file-assets`);
            expect(response.status).toBe(401);
        });
        it('should prevent access to assets from other clients', async () => {
            const otherClientId = 999; // Non-existent client
            const response = await authenticatedFetch(`${API_BASE}/clients/${otherClientId}/file-assets`);
...
export { createTestFile, authenticatedFetch, API_BASE, };
