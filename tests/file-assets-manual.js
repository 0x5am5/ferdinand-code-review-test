...
async function main() {
    ...
    await runTest('Upload file successfully', async () => {
        ...
        const response = await fetch(`${API_BASE}/clients/${TEST_CLIENT_ID}/file-assets/upload`, {
            method: 'POST',
            body: formData,
        });
    ...
    await runTest('List assets with pagination', async () => {
        const response = await fetch(`${API_BASE}/clients/${TEST_CLIENT_ID}/file-assets?limit=10&offset=0`);
        const data = await response.json();
    ...
    await runTest('Get single asset by ID', async () => {
        const response = await fetch(`${API_BASE}/clients/${TEST_CLIENT_ID}/file-assets/${testAssetId}`);
        const data = await response.json();
    ...
    await runTest('Download asset file', async () => {
        const response = await fetch(`${API_BASE}/clients/${TEST_CLIENT_ID}/file-assets/${testAssetId}/download`);
        assertEqual(response.status, 200, 'Download failed');
        assert(response.headers.get('Content-Type'), 'Content-Type header missing');
        assert(response.headers.get('Content-Disposition')?.includes('attachment'), 'Content-Disposition not set to attachment');
    ...
    await runTest('Create asset category', async () => {
        const response = await fetch(`${API_BASE}/clients/${TEST_CLIENT_ID}/file-asset-categories`, {
            method: 'POST',
    ...
    await runTest('Create asset tag', async () => {
        const response = await fetch(`${API_BASE}/clients/${TEST_CLIENT_ID}/file-asset-tags`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                name: 'Test Tag',
    ...
    await runTest('Filter assets by visibility', async () => {
        const response = await fetch(`${API_BASE}/clients/${TEST_CLIENT_ID}/file-assets?visibility=private`);
        const data = await response.json();
    ...
    await runTest('Soft delete asset', async () => {
        ...
        const getResponse = await fetch(`${API_BASE}/clients/${TEST_CLIENT_ID}/file-assets/${testAssetId}`);
        assert(getResponse.status === 403 || getResponse.status === 404, 'Deleted asset still accessible');
    });
    // Test 12: Cleanup - delete test category
    if (testCategoryId) {
        await runTest('Delete test category', async () => {
            const response = await fetch(`${API_BASE}/clients/${TEST_CLIENT_ID}/file-asset-categories/${testCategoryId}`, {
                method: 'DELETE',
            });
            if (response.status === 403) {
                console.log('  ⚠️  Skipped - requires admin role');
    ...
    if (testTagId) {
        await runTest('Delete test tag', async () => {
            const response = await fetch(`${API_BASE}/clients/${TEST_CLIENT_ID}/file-asset-tags/${testTagId}`, {
                method: 'DELETE',
            });
            if (response.status === 403) {
                console.log('  ⚠️  Skipped - requires admin role');
