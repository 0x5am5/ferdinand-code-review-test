...
async function runTest() {
  try {
    ...
    console.log('API_KEY appears to be a valid SendGrid key:', !isEmailInApiKeyField);
    console.log('FROM_EMAIL appears to be a valid email address:', !isApiKeyInFromEmailField);
    ...
    if (isApiKeyInFromEmailField) {
      console.error('ERROR: FROM_EMAIL appears to contain an API key, not an email address.');
      console.error('Please check your environment variables.');
      process.exit(1);
    }
    
    if (isEmailInApiKeyField) {
      console.error('ERROR: SENDGRID_API_KEY appears to contain an email address, not an API key.');
      console.error('Please check your environment variables.');
      process.exit(1);
    }
