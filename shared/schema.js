...
import { boolean, index, integer, json, jsonb, pgTable, serial, text, timestamp, varchar, } from "drizzle-orm/pg-core";
...
export const slackUserMappings = pgTable("slack_user_mappings", {
    ...
    slackUserTeamIdx: index("idx_slack_user_mappings_user_team_active").on(table.slackUserId, table.slackTeamId, table.isActive),
...
export const assetTags = pgTable("asset_tags", {
    ...
    uniqueNamePerClient: index("idx_asset_tags_unique_name_client").on(sql `LOWER(${table.name})`, table.clientId),
}));
export const assetCategoryAssignments = pgTable("asset_category_assignments", {
    ...
    pk: index("pk_asset_category_assignments").on(table.assetId, table.categoryId),
    assetIdIdx: index("idx_asset_category_assignments_asset").on(table.assetId),
    categoryIdIdx: index("idx_asset_category_assignments_category").on(table.categoryId),
}));
...
export const assetPublicLinks = pgTable("asset_public_links", {
    id: serial("id").primaryKey(),
    assetId: integer("asset_id")
        .notNull()
        .references(() => assets.id),
    ...
    expiresAtIdx: index("idx_asset_public_links_expires_at").on(table.expiresAt),
}));
// Insert Schemas
...
export const insertBrandAssetSchema = createInsertSchema(brandAssets)
    ...
    data: z.object({
        type: z.enum(Object.values(LogoType)),
        format: z.enum(Object.values(FILE_FORMATS)),
        fileName: z.string(),
    }),
    fileData: z.string(),
    mimeType: z.string(),
...
export const insertColorAssetSchema = createInsertSchema(brandAssets)
    ...
    data: z.object({
        ...
        category: z.enum(Object.values(ColorCategory)),
        colors: z
            .array(z.object({
            hex: z.string(),
...
export const insertFontAssetSchema = createInsertSchema(brandAssets)
    ...
    data: z.object({
        source: z.enum(Object.values(FontSource)),
        weights: z.array(z.enum(Object.values(FontWeight))),
        styles: z.array(z.enum(Object.values(FontStyle))),
        sourceData: z.object({
            projectId: z.string().optional(),
            url: z.string().url().optional(),
            files: z
                .array(z.object({
                weight: z.enum(Object.values(FontWeight)),
                style: z.enum(Object.values(FontStyle)),
...
export const insertUserPersonaSchema = createInsertSchema(userPersonas)
    ...
    demographics: z
        ...
        .optional(),
    eventAttributes: z.array(z.enum(Object.values(PersonaEventAttribute))),
    metrics: z
        .object({
...
export const insertInspirationSectionSchema = createInsertSchema(inspirationSections).omit({ id: true, createdAt: true, updatedAt: true });
export const insertInspirationImageSchema = createInsertSchema(inspirationImages).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertHiddenSectionSchema = createInsertSchema(hiddenSections).omit({ id: true, createdAt: true });
...
export const assetCategoryAssignmentsRelations = relations(assetCategoryAssignments, ({ one }) => ({
    asset: one(assets, {
...
export const assetTagAssignmentsRelations = relations(assetTagAssignments, ({ one }) => ({
    asset: one(assets, {
...
export const assetPublicLinksRelations = relations(assetPublicLinks, ({ one }) => ({
    asset: one(assets, {
...
export const insertAssetSchema = createInsertSchema(assets)
    ...
    message: "For reference-only assets, fileSize must be 0 and storagePath must be empty. For regular assets, fileSize must be >= 1 and storagePath must not be empty.",
...
export const insertAssetCategoryAssignmentSchema = createInsertSchema(assetCategoryAssignments).extend({
    assetId: z.number(),
    categoryId: z.number(),
});
export const insertAssetTagAssignmentSchema = createInsertSchema(assetTagAssignments).extend({
    assetId: z.number(),
    tagId: z.number(),
});
