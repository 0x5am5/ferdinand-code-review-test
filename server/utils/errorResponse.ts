...
export function parsePostmarkError(error: any): {
  ...
  const statusCode =
    error.statusCode || (typeof error.code === "number" ? error.code : null);
  if (statusCode) {
...
export function parseSendGridError(error: unknown): {
  ...
  if (
    ...
    if (
      (typeof statusCode === "number" && statusCode === 402) ||
      (body &&
        typeof body === "object" &&
        "errors" in body &&
    ...
    if (
      ...
      const firstError =
        body.errors[0] instanceof Error ? body.errors[0] : body.errors[0];
      return {
