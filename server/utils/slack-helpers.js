...
export function findBestLogoMatch(assets, query = "") {
    if (!query.trim()) {
        return assets;
    }
    ...
    const nameMatches = assets.filter((asset) => asset.name.toLowerCase().includes(queryLower));
...
export function generateAssetDownloadUrl(assetId, clientId, baseUrl, options = {}) {
...
export function decryptBotToken(encryptedToken) {
    try {
...
export async function uploadFileToSlack(botToken, options) {
    try {
        ...
        try {
            ...
            const isChannelAccessError = error.data?.error === "not_in_channel" ||
                error.data?.error === "channel_not_found" ||
                error.data?.error === "access_denied" ||
                error.data?.error === "missing_scope";
            if (isChannelAccessError && options.userId) {
                console.log(`[SLACK UPLOAD] Bot cannot access channel (${error.data?.error}), falling back to DM to user ${options.userId}`);
                try {
                    ...
                    const err = dmError;
                    console.error(`[SLACK UPLOAD] DM upload failed:`, err.message || dmError);
                    ...
                    try {
                        ...
                        console.error(`[SLACK UPLOAD] Download link message also failed:`, linkErr.message || linkError);
                    }
                    return false;
                }
            }
            // Re-throw if it's not a channel access issue or no userId provided
            console.error(`[SLACK UPLOAD] Channel upload failed with non-access error or no userId provided: ${error.data?.error}`);
            throw channelError;
        }
    }
    catch (error) {
        const err = error;
        console.error("[SLACK UPLOAD] Error uploading file to Slack:", err.message || error);
        return false;
    }
}
// Format asset information for Slack blocks
export function formatAssetInfo(asset) {
    try {
        const data = typeof asset.data === "string" ? JSON.parse(asset.data) : asset.data;
...
export async function validateSlackRequest(signingSecret, requestSignature, timestamp, body) {
...
export function checkRateLimit(workspaceId, maxRequests = 10, windowMs = 60000) {
    const now = Date.now();
...
export async function logSlackActivity(entry) {
...
export function generateColorSwatchUrl(colors) {
    if (colors.length === 0) {
...
export function formatColorInfo(colorAsset) {
    try {
        const data = typeof colorAsset.data === "string"
...
export function filterColorAssetsByVariant(colorAssets, variant = "") {
    if (!variant.trim()) {
        return colorAssets;
    }
    ...
    return colorAssets.filter((asset) => {
        ...
        try {
            ...
            const categoryMatch = targetKeywords.some((keyword) => category.includes(keyword) || type.includes(keyword));
...
export function filterFontAssetsByVariant(assets, variant) {
    if (!variant)
        return assets;
    ...
    return assets.filter((asset) => {
        ...
        return (name.includes(lowerVariant) ||
...
export function formatFontInfo(fontAsset) {
    try {
        const data = typeof fontAsset.data === "string"
...
export function generateGoogleFontCSS(fontFamily, weights) {
...
export function generateAdobeFontCSS(projectId, fontFamily) {
...
export function hasUploadableFiles(fontAsset) {
    try {
        const data = typeof fontAsset.data === "string"
            ? JSON.parse(fontAsset.data)
            : fontAsset.data;
        // Check multiple conditions for uploadable files
        const hasSourceFiles = data?.source === "file" &&
            data?.sourceData?.files &&
            data.sourceData.files.length > 0;
        const hasCustomFiles = data?.sourceData?.files &&
            data.sourceData.files.length > 0 &&
            data?.source !== "google" &&
            data?.source !== "adobe";
