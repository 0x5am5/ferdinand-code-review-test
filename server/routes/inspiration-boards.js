import { insertInspirationImageSchema, insertInspirationSectionSchema, } from "@shared/schema";
...
const upload = multer();
export function registerInspirationBoardsRoutes(app) {
    app.get("/api/clients/:clientId/inspiration/sections", validateClientId, async (req, res) => {
        try {
            ...
            console.error("Error fetching inspiration sections:", error instanceof Error ? error.message : "Unknown error");
            res
                .status(500)
                .json({ message: "Error fetching inspiration sections" });
        }
    });
    app.post("/api/clients/:clientId/inspiration/sections", validateClientId, requireAdminRole, async (req, res) => {
        try {
            const clientId = req.clientId;
            if (!clientId) {
                return res.status(400).json({ message: "Client ID is required" });
            ...
            console.error("Error creating inspiration section:", error instanceof Error ? error.message : "Unknown error");
            res.status(500).json({ message: "Error creating inspiration section" });
        }
    });
    app.patch("/api/clients/:clientId/inspiration/sections/:sectionId", validateClientId, requireAdminRole, async (req, res) => {
        try {
            const clientId = req.clientId;
            if (!clientId) {
                return res.status(400).json({ message: "Client ID is required" });
            ...
            console.error("Error updating inspiration section:", error instanceof Error ? error.message : "Unknown error");
            res.status(500).json({ message: "Error updating inspiration section" });
        }
    });
    app.delete("/api/clients/:clientId/inspiration/sections/:sectionId", validateClientId, requireAdminRole, async (req, res) => {
        try {
            const clientId = req.clientId;
            if (!clientId) {
                return res.status(400).json({ message: "Client ID is required" });
            ...
            console.error("Error deleting inspiration section:", error instanceof Error ? error.message : "Unknown error");
    ...
    app.post("/api/clients/:clientId/inspiration/sections/:sectionId/images", upload.single("image"), validateClientId, async (req, res) => {
        try {
            ...
            console.error("Error uploading inspiration image:", error instanceof Error ? error.message : "Unknown error");
