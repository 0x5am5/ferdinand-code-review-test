import { brandAssets, convertedAssets, insertColorAssetSchema, insertFontAssetSchema, } from "@shared/schema";
...
export function registerAssetRoutes(app) {
    ...
    app.get("/api/adobe-fonts/:projectId", async (req, res) => {
        try {
            ...
            const transformedData = {
                ...
                fonts: data.kit?.families
                    ...
                    return {
                        ...
                        weights: weights.sort((a, b) => parseInt(a, 10) - parseInt(b, 10)),
                        styles: styles,
            ...
            console.error("Error fetching Adobe Fonts:", error instanceof Error ? error.message : "Unknown error");
            res.status(500).json({
    ...
    app.get("/api/google-fonts", async (_req, res) => {
        try {
            ...
            console.error("Error fetching Google Fonts:", error instanceof Error ? error.message : "Unknown error");
    ...
    app.get("/api/brand-assets", async (req, res) => {
        try {
            ...
            console.error("Error fetching brand assets:", error instanceof Error ? error.message : "Unknown error");
    ...
    app.post("/api/clients/:clientId/assets", upload.any(), validateClientId, async (req, res) => {
        try {
            ...
            if (category === "font") {
                ...
                if (subcategory === "google") {
                    ...
                    try {
                        ...
                        console.error("Error parsing font data:", error instanceof Error ? error.message : "Unknown error");
                        return res
                            .status(400)
                    ...
                    const fontAsset = {
                        ...
                        data: {
                            ...
                            sourceData: fontData.sourceData,
                        },
                ...
                if (subcategory === "adobe") {
                    ...
                    try {
                        ...
                        console.error("Error parsing Adobe font data:", error instanceof Error ? error.message : "Unknown error");
                        return res
                            .status(400)
                            .json({ message: "Invalid font data format" });
                    }
            ...
            if (category !== "font") {
                ...
                try {
                    ...
                    for (const convertedFile of convertedFiles) {
                        ...
                        if (convertedFile.format === originalFormat)
                            continue;
                    ...
                    console.error("Error converting asset to other formats:", conversionError instanceof Error
            ...
            console.error("Error creating asset:", error instanceof Error ? error.message : "Unknown error");
            res.status(500).json({ message: "Error creating asset" });
        }
    });
    // Update asset endpoint
    app.patch("/api/clients/:clientId/assets/:assetId", upload.any(), validateClientId, async (req, res) => {
        try {
            const clientId = req.clientId;
            if (!clientId) {
                return res.status(400).json({ message: "Client ID is required" });
            ...
            if (req.body.category === "font") {
                ...
                const files = req.files;
                const isDarkVariant = req.body.isDarkVariant === "true" ||
                    req.body.isDarkVariant === true;
                if (!files || files.length === 0) {
                    return res.status(400).json({ message: "No file uploaded" });
                }
                ...
                const existingData = typeof asset.data === "string"
                    ? JSON.parse(asset.data)
                    : asset.data;
                if (isDarkVariant) {
                ...
                `);
                            ...
                            console.error("Error deleting existing dark variant converted assets:", deleteError instanceof Error
                                ? deleteError.message
                                : "Unknown error");
                        }
                        // Store all converted versions in the database
                        for (const convertedFile of convertedFiles) {
                            ...
                            const existingConverted = await storage.getConvertedAsset(asset.id, convertedFile.format, true // isDarkVariant
                            );
                        ...
                        console.error("Error converting dark variant to other formats:", conversionError instanceof Error
                            ? conversionError.message
                            : "Unknown error");
                ...
                `);
                            ...
                            console.error("Error deleting existing light variant converted assets:", deleteError instanceof Error
                                ? deleteError.message
                                : "Unknown error");
                        }
                        // Store all converted versions in the database
                        ...
                        console.error("Error converting light variant to other formats:", conversionError instanceof Error
                            ? conversionError.message
                            : "Unknown error");
                        // We'll continue even if conversion fails since the original update will succeed
                    }
            ...
            try {
                ...
                console.error("Error updating client logo during asset update:", logoUpdateError instanceof Error
    ...
    app.delete("/api/clients/:clientId/assets/:assetId", validateClientId, async (req, res) => {
        try {
            ...
            `);
                    ...
                    console.error("Error deleting converted assets for dark variant:", error instanceof Error ? error.message : "Unknown error");
    ...
    app.get("/api/clients/:clientId/assets", validateClientId, async (req, res) => {
        try {
            ...
            console.error("Error fetching client assets:", error instanceof Error ? error.message : "Unknown error");
            res.status(500).json({ message: "Error fetching client assets" });
        }
    });
    // New optimized asset serving endpoints for better caching
    app.get("/api/assets/:assetId/light", async (req, res) => {
    ...
    app.get("/api/assets/:assetId/dark", async (req, res) => {
    ...
    app.get("/api/assets/:assetId/file", async (req, res) => {
        ...
        try {
            ...
            if (format && asset.category === "logo") {
                ...
                console.log(`Asset details - Name: ${asset.name}, ID: ${asset.id}, Client ID: ${asset.clientId}, Variant: ${variant}, Dark: ${isDarkVariant}`);
                ...
                if (isDarkVariant) {
                    ...
                    if (!hasDarkVariant && !hasDarkVariantFileData) {
                        ...
                        const convertedAsset = await storage.getConvertedAsset(assetId, format, false);
                        if (convertedAsset &&
            ...
            const isVectorFormat = (mimeType &&
                ["image/svg+xml", "application/postscript", "application/pdf"].some((type) => mimeType?.includes(type))) ||
            ...
            if (size &&
                ...
                try {
                    ...
                    console.error("Error handling vector-to-raster conversion:", vectorError instanceof Error
                        ? vectorError.message
                        : "Unknown error");
                }
            }
            ...
            if (size &&
                ...
                try {
                    ...
                    console.error("Image resize failed:", resizeError instanceof Error
                        ? resizeError.message
                        : "Unknown error");
            ...
            console.error("Error serving asset file:", error instanceof Error ? error.message : "Unknown error");
            res.status(500).json({ message: "Error serving asset file" });
        }
    });
    ...
    function getDarkVariantBuffer(asset) {
        ...
        try {
            ...
            console.error("Error parsing dark variant data:", error instanceof Error ? error.message : "Unknown error");
        }
        return null;
    }
    // Helper function to check if a mimetype is an image format
    function isImageFormat(mimeType) {
    ...
    async function convertToFormat(buffer, sourceFormat, targetFormat, assetId) {
        if (!buffer) {
        ...
        try {
            ...
            console.error("Error in format conversion:", error instanceof Error ? error.message : "Unknown error");
            throw new Error(`Failed to convert ${sourceFormat} to ${targetFormat}: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    ...
    app.get("/api/clients/:clientId/assets/:assetId/download", validateClientId, async (req, res) => {
        // Redirect to the existing /file endpoint with the same parameters
        const assetId = req.params.assetId;
        const query = new URLSearchParams(req.query);
        const queryString = query.toString();
    ...
    app.get("/api/assets/:assetId/converted", async (req, res) => {
        try {
            const assetId = parseInt(req.params.assetId, 10);
            const asset = await storage.getAsset(assetId);
            if (!asset) {
            ...
            console.error("Error fetching converted assets:", error instanceof Error ? error.message : "Unknown error");
            res.status(500).json({ message: "Error fetching converted assets" });
        }
    });
    // Get thumbnail for an asset
    app.get("/api/assets/:assetId/thumbnail/:size", async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            const [asset] = await db
                ...
                .where(and(eq((await import("@shared/schema")).assets.id, assetId), isNull((await import("@shared/schema")).assets.deletedAt), inArray((await import("@shared/schema")).assets.clientId, clientIds)));
            if (!asset) {
                return res.status(404).json({ message: "Asset not found" });
            }
