import { assetCategoryAssignments, assetPublicLinks, assets, assetTagAssignments, assetTags, insertAssetSchema, insertAssetTagSchema, UserRole, } from "@shared/schema";
...
import { ensureDefaultCategories, getCategoriesForClient, } from "../services/default-categories";
import { deleteThumbnails } from "../services/thumbnail";
import { deleteFile, downloadFile, generateStoragePath, generateUniqueFileName, uploadFile, validateFileSize, validateMimeType, } from "../storage/index";
import { autoSelectCategory, determineAssetCategory, } from "../utils/asset-categorization";
export function registerFileAssetRoutes(app) {
    ...
    app.get("/api/assets", async (req, res) => {
        try {
            ...
            const tagIds = req.query.tagIds
                ? req.query.tagIds.split(",").map((id) => parseInt(id, 10))
                : undefined;
            ...
            if (categoryId) {
                const rows = await db
                    .select({ asset: assets })
                    .from(assets)
                    .innerJoin(assetCategoryAssignments, eq(assets.id, assetCategoryAssignments.assetId))
                    .where(and(...conditions, eq(assetCategoryAssignments.categoryId, categoryId)));
            ...
            // Filter by tags if specified (assets must have ALL specified tags)
                ...
                const assetsWithAllTags = await Promise.all(assetList.map(async (asset) => {
                ...
                assetList = assetsWithAllTags.filter((a) => a !== null);
            }
            else {
            ...
            const assetsWithDetails = await Promise.all(assetList.map(async (asset) => {
                // Get categories
                const categoryRows = await db
                    ...
                    .innerJoin((await import("@shared/schema")).assetCategories, eq(assetCategoryAssignments.categoryId, (await import("@shared/schema")).assetCategories.id))
    ...
    app.get("/api/assets/search", async (req, res) => {
        try {
            ...
            const searchResults = await db
                .select({
                asset: assets,
                rank: sql `ts_rank(to_tsvector('english', ${assets.fileName} || ' ' || ${assets.originalFileName}), to_tsquery('english', ${searchQuery}))`,
            })
                .from(assets)
                .where(and(...conditions))
            ...
            console.error("Error searching assets:", error instanceof Error ? error.message : "Unknown error");
    ...
    app.get("/api/assets/:assetId", async (req, res) => {
        try {
            ...
            const [asset] = await db
                .select()
                .from(assets)
                .where(and(eq(assets.id, assetId), isNull(assets.deletedAt), inArray(assets.clientId, clientIds)));
            if (!asset) {
                return res.status(404).json({ message: "Asset not found" });
            }
            // Check if user has read permission
            const permission = await checkAssetPermission(req.session.userId, assetId, asset.clientId, "read");
            ...
            console.error("Error fetching asset:", error instanceof Error ? error.message : "Unknown error");
    ...
    app.get("/api/assets/:assetId/download", async (req, res) => {
        try {
            ...
            if (downloadAsset.referenceOnly) {
                ...
                console.log(`Redirecting to Google Drive file: ${downloadAsset.driveWebLink}`);
            ...
            if (!downloadAsset.storagePath) {
                console.error(`ERROR: Asset ${assetId} has empty storagePath and is not a reference asset`);
                return res.status(400).json({
            ...
            res.setHeader("Content-Disposition", `attachment; filename="${downloadAsset.originalFileName}"`);
            res.setHeader("Content-Length", fileBuffer.length);
            res.send(fileBuffer);
        }
        catch (error) {
            console.error("Error downloading asset:", error instanceof Error ? error.message : "Unknown error");
    ...
    app.patch("/api/assets/:assetId", async (req, res) => {
        try {
            ...
            const permission = await checkAssetPermission(req.session.userId, assetId, asset.clientId, "write");
            if (!permission.allowed) {
                return res
                    .status(403)
            ...
            if (categories !== undefined) {
                ...
                if (categoryIds.length > 0) {
                    await db.insert(assetCategoryAssignments).values(categoryIds.map((categoryId) => ({
                        assetId,
                        categoryId,
                    })));
                }
            ...
            if (tags !== undefined) {
                ...
                // Add new tags
                if (tagIds.length > 0) {
                    await db.insert(assetTagAssignments).values(tagIds.map((tagId) => ({
                        assetId,
                        tagId,
                    })));
                }
            ...
            console.error("Error updating asset:", error instanceof Error ? error.message : "Unknown error");
    ...
    app.delete("/api/assets/:assetId", async (req, res) => {
        try {
            ...
            const permission = await checkAssetPermission(req.session.userId, assetId, asset.clientId, "delete");
            if (!permission.allowed) {
                return res
                    .status(403)
            ...
            try {
                ...
                console.error(`Failed to delete thumbnails for asset ${assetId}:`, error);
            }
            res.json({ message: "Asset deleted successfully" });
        }
        catch (error) {
            console.error("Error deleting asset:", error instanceof Error ? error.message : "Unknown error");
    ...
    app.post("/api/assets/bulk-delete", async (req, res) => {
        try {
            ...
            const assetsToDelete = await db
                .select()
                .from(assets)
                .where(and(inArray(assets.id, assetIds), isNull(assets.deletedAt), inArray(assets.clientId, clientIds)));
            if (assetsToDelete.length === 0) {
                return res.status(404).json({ message: "No assets found" });
            }
            // Check permissions for each asset
            const userId = req.session.userId;
            const permissionChecks = await Promise.all(assetsToDelete.map((asset) => checkAssetPermission(userId, asset.id, asset.clientId, "delete")));
            ...
            await Promise.all(assetsToDelete.map(async (asset) => {
                ...
                try {
                    ...
                    console.error(`Failed to delete thumbnails for asset ${asset.id}:`, error);
                }
            }));
            res.json({
            ...
            console.error("Error bulk deleting assets:", error instanceof Error ? error.message : "Unknown error");
    ...
    app.post("/api/assets/bulk-update", async (req, res) => {
        try {
            ...
            const userId = req.session.userId;
            const permissionChecks = await Promise.all(assetsToUpdate.map((asset) => checkAssetPermission(userId, asset.id, asset.clientId, "write")));
            const unauthorizedAssets = permissionChecks.filter((p) => !p.allowed);
            if (unauthorizedAssets.length > 0) {
                return res
                    .status(403)
            ...
            if (removeTags !== undefined &&
                ...
                await db
                    .delete(assetTagAssignments)
                    .where(and(inArray(assetTagAssignments.assetId, assetIds), inArray(assetTagAssignments.tagId, removeTags)));
            }
            ...
            console.error("Error bulk updating assets:", error instanceof Error ? error.message : "Unknown error");
            res.status(500).json({ message: "Error bulk updating assets" });
        }
    });
    // Global upload endpoint (infers clientId from session/user context)
    app.post("/api/assets/upload", csrfProtection, uploadRateLimit, upload.single("file"), virusScan, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            if (finalCategoryIds.length === 0) {
                try {
                    ...
                    const autoCategoryId = autoSelectCategory(file.originalname, file.mimetype, categories);
                    if (autoCategoryId) {
                        finalCategoryIds = [autoCategoryId];
                        const categoryName = determineAssetCategory(file.originalname, file.mimetype);
                        console.log(`Auto-categorized uploaded file "${file.originalname}" as "${categoryName}" (ID: ${autoCategoryId})`);
                    }
                }
                catch (categoryError) {
                    console.warn("Failed to auto-categorize uploaded file:", categoryError);
            ...
            // Assign categories
            if (finalCategoryIds.length > 0) {
                await db.insert(assetCategoryAssignments).values(finalCategoryIds.map((categoryId) => ({
                    assetId: asset.id,
                    categoryId,
                })));
            }
            ...
            if (tagNames.length > 0) {
                ...
                for (const tagName of tagNames) {
                    ...
                    const [existingTag] = await db
                        .select()
                        .from(assetTags)
                        .where(and(eq(assetTags.clientId, clientId), eq(assetTags.slug, slug)));
                    if (existingTag) {
            ...
            console.error("Error uploading asset:", error instanceof Error ? error.message : "Unknown error");
            res.status(500).json({ message: "Error uploading asset" });
        }
    });
    // Upload asset (client-scoped)
    app.post("/api/clients/:clientId/file-assets/upload", csrfProtection, uploadRateLimit, validateClientId, upload.single("file"), virusScan, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
    ...
    app.get("/api/clients/:clientId/file-assets", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            // Filter by tag if specified
                ...
                const rows = await db
                    .select({ asset: assets })
                    .from(assets)
                    .innerJoin(assetTagAssignments, eq(assets.id, assetTagAssignments.assetId))
            ...
            const sortedAssets = assetList.sort((a, b) => new Date(b.createdAt ?? new Date()).getTime() -
    ...
    // Get single asset
    app.get("/api/clients/:clientId/file-assets/:assetId", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            const assetId = parseInt(req.params.assetId, 10);
            const permission = await checkAssetPermission(req.session.userId, assetId, clientId, "read");
            if (!permission.allowed || !permission.asset) {
                return res
                    .status(403)
                    .json({ message: "Not authorized to view this asset" });
    ...
    // Download asset
    app.get("/api/clients/:clientId/file-assets/:assetId/download", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
    ...
    // Update asset metadata
    app.patch("/api/clients/:clientId/file-assets/:assetId", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            const assetId = parseInt(req.params.assetId, 10);
            const permission = await checkAssetPermission(req.session.userId, assetId, clientId, "write");
            if (!permission.allowed || !permission.asset) {
                return res
                    .status(403)
                    .json({ message: "Not authorized to update this asset" });
    ...
    app.get("/api/clients/:clientId/file-assets/:assetId/thumbnail/:size", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            const { canGenerateThumbnail, getOrGenerateThumbnail, getFileTypeIcon, } = await import("../services/thumbnail");
            ...
            try {
                ...
                const _thumbnailStoragePath = await getOrGenerateThumbnail(tempFilePath, assetId, size, asset.fileType || "");
    ...
    app.delete("/api/clients/:clientId/file-assets/:assetId", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            const assetId = parseInt(req.params.assetId, 10);
            const permission = await checkAssetPermission(req.session.userId, assetId, clientId, "delete");
            if (!permission.allowed || !permission.asset) {
                return res
                    .status(403)
                    .json({ message: "Not authorized to delete this asset" });
    ...
    app.post("/api/clients/:clientId/assets/:assetId/public-links", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            const permission = await checkAssetPermission(req.session.userId, assetId, clientId, "share");
            if (!permission.allowed || !permission.asset) {
                return res
                    .status(403)
            ...
            console.error("Error creating public link:", error instanceof Error ? error.message : "Unknown error");
            res.status(500).json({ message: "Error creating public link" });
        }
    });
    // List public links for an asset
    app.get("/api/clients/:clientId/assets/:assetId/public-links", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            console.error("Error fetching public links:", error instanceof Error ? error.message : "Unknown error");
            res.status(500).json({ message: "Error fetching public links" });
        }
    });
    // Delete a public link
    app.delete("/api/clients/:clientId/assets/:assetId/public-links/:linkId", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            await db
                .delete(assetPublicLinks)
                .where(and(eq(assetPublicLinks.id, linkId), eq(assetPublicLinks.assetId, assetId)));
            res.json({ message: "Public link deleted successfully" });
        }
        catch (error) {
            console.error("Error deleting public link:", error instanceof Error ? error.message : "Unknown error");
            res.status(500).json({ message: "Error deleting public link" });
        }
    });
    // Admin endpoint to fix missing reference-only asset links
    app.post("/api/admin/fix-reference-assets", async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            for (const asset of missingLinkAssets) {
                if (asset.driveFileId) {
                    ...
                    try {
                        ...
                        console.log(`Fixed asset ${asset.id} (${asset.originalFileName}) with link: ${webLink}`);
                    }
                    catch (updateError) {
    ...
    app.get("/api/public/assets/:token", async (req, res) => {
        try {
            ...
            const [asset] = await db
                .select()
                .from(assets)
                .where(and(eq(assets.id, publicLink.assetId), isNull(assets.deletedAt)));
            if (!asset) {
                return res.status(404).json({ message: "Asset not found" });
            }
            // Check if this is a reference-only asset (Google Workspace file)
            ...
            // Validate storagePath exists for regular assets
            if (!asset.storagePath) {
                console.error(`ERROR: Asset has empty storagePath and is not a reference asset`);
                return res.status(400).json({
                    message: "Asset storage path not found",
                });
            }
            ...
            console.error("Error serving public asset:", error instanceof Error ? error.message : "Unknown error");
