...
export function registerApiTokenRoutes(app) {
    ...
    app.post("/api/clients/:clientId/tokens", csrfProtection, tokenCreationRateLimit, validateClientId, async (req, res) => {
        try {
            ...
            const tokenScopes = scopes && Array.isArray(scopes) ? scopes : ["read:assets"];
            const tokenData = {
                clientId,
                tokenHash,
                tokenName,
    ...
    app.get("/api/clients/:clientId/tokens", validateClientId, async (req, res) => {
        try {
            const clientId = req.clientId;
            if (!clientId) {
                return res.status(400).json({ message: "Client ID is required" });
    ...
    app.patch("/api/clients/:clientId/tokens/:tokenId", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
    ...
    app.delete("/api/clients/:clientId/tokens/:tokenId", validateClientId, async (req, res) => {
        try {
            const clientId = req.clientId;
            const tokenId = parseInt(req.params.tokenId, 10);
            if (!clientId || !tokenId) {
    ...
    app.get("/api/clients/:clientId/tokens/:tokenId/stats", validateClientId, async (req, res) => {
        try {
            const clientId = req.clientId;
            const tokenId = parseInt(req.params.tokenId, 10);
            if (!clientId || !tokenId) {
