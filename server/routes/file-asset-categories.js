import { assetCategories, insertAssetCategorySchema, UserRole, } from "@shared/schema";
...
export function registerFileAssetCategoryRoutes(app) {
    ...
    app.get("/api/asset-categories", async (req, res) => {
        try {
            ...
            console.error("Error fetching asset categories:", error instanceof Error ? error.message : "Unknown error");
            res.status(500).json({ message: "Error fetching asset categories" });
        }
    });
    // Get all categories (system defaults + client-specific)
    app.get("/api/clients/:clientId/file-asset-categories", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
    ...
    app.post("/api/clients/:clientId/file-asset-categories", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            console.error("Error creating asset category:", error instanceof Error ? error.message : "Unknown error");
            res.status(500).json({ message: "Error creating asset category" });
        }
    });
    // Update a category (admin only)
    app.patch("/api/clients/:clientId/file-asset-categories/:categoryId", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            console.error("Error updating asset category:", error instanceof Error ? error.message : "Unknown error");
            res.status(500).json({ message: "Error updating asset category" });
        }
    });
    // Delete a category (admin only)
    app.delete("/api/clients/:clientId/file-asset-categories/:categoryId", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            console.error("Error deleting asset category:", error instanceof Error ? error.message : "Unknown error");
