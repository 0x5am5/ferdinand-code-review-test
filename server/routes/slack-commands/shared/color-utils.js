...
export function buildColorBlocks(displayAssets, filteredColorAssets, colorAssets, variant, formatColorInfo) {
    ...
    for (const [category, assets] of Object.entries(groupedAssets)) {
        if (categoryOrder.includes(category) || assets.length === 0)
            continue;
        colorBlocks.push({
            type: "section",
            text: {
