...
export async function handleUnifiedCommand({ command, ack, respond, }) {
    await ack();
        ...
        if (!isTraditionalCommand) {
            ...
            const [logoAssets, colorAssets, fontAssets] = await Promise.all([
                db
                    .select()
                    .from(brandAssets)
                    .where(and(eq(brandAssets.clientId, workspaceData.clientId), eq(brandAssets.category, "logo"))),
                db
                    .select()
                    .from(brandAssets)
                    .where(and(eq(brandAssets.clientId, workspaceData.clientId), eq(brandAssets.category, "color"))),
                db
                    .select()
                    .from(brandAssets)
                    .where(and(eq(brandAssets.clientId, workspaceData.clientId), eq(brandAssets.category, "font"))),
            ]);
