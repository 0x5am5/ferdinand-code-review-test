...
import { db } from "../../../db";
import { buildColorBlocks, buildColorConfirmationBlocks, shouldShowColorConfirmation, } from "../../../utils/color-display";
import { filterColorAssetsByVariant, logSlackActivity, } from "../../../utils/slack-helpers";
export async function handleColorSubcommand({ respond, variant, workspace, auditLog, }) {
    ...
    if (shouldShowColorConfirmation(displayAssets.length)) {
        const confirmationBlocks = buildColorConfirmationBlocks(displayAssets, variant, workspace.clientId);
        await respond({
    ...
    const colorBlocks = buildColorBlocks(displayAssets, filteredColorAssets, colorAssets, variant);
    await respond({
        blocks: colorBlocks,
        response_type: "ephemeral",
    });
