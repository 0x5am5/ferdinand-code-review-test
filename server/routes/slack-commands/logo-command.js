...
import { db } from "../../db";
import { buildLogoConfirmationBlocks, buildLogoProcessingMessage, buildLogoSummaryMessage, shouldShowLogoConfirmation, } from "../../utils/logo-display";
import { checkRateLimit, decryptBotToken, findBestLogoMatch, formatAssetInfo, generateAssetDownloadUrl, logSlackActivity, uploadFileToSlack, } from "../../utils/slack-helpers";
export async function handleLogoCommand({ command, ack, respond, }) {
    ...
    try {
        ...
        if (shouldShowLogoConfirmation(matchedLogos.length)) {
            const confirmationBlocks = buildLogoConfirmationBlocks(matchedLogos, query, workspace.clientId);
            await respond({
            ...
            try {
                ...
                for (const asset of matchedLogos) {
                    ...
                    if (isDarkQuery) {
                        ...
                        const lightUrl = generateAssetDownloadUrl(asset.id, workspace.clientId, baseUrl, lightParams);
                        ...
                        if (hasDarkVariant) {
                            ...
                            const darkUrl = generateAssetDownloadUrl(asset.id, workspace.clientId, baseUrl, darkParams);
                ...
                if (successfulUploads === 0) {
                    ...
                    const summaryText = buildLogoSummaryMessage(successfulUploads, matchedLogos.length, query, responseTime);
                    try {
                        await workspaceClient.chat.postEphemeral({
                            channel: command.channel_id,
                            user: command.user_id,
