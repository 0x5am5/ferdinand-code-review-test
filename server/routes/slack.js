...
import { insertSlackUserMappingSchema, slackUserMappings, slackWorkspaces, } from "@shared/schema";
import * as dotenv from "dotenv";
import { and, eq, or } from "drizzle-orm";
import { validateClientId } from "server/middlewares/vaildateClientId";
import { db } from "../db";
import { storage } from "../storage";
import { handleColorSubcommandWithLimit, handleFontSubcommandWithLimit, handleLogoSubcommandWithLimit, } from "./slack-commands/action-handlers";
...
function initializeSlackApp() {
    if (slackApp)
        return slackApp; // Already initialized
    ...
    if (process.env.SLACK_BOT_TOKEN && process.env.SLACK_SIGNING_SECRET) {
        ...
        slackApp.action("show_all_colors", async ({ ack, body, respond }) => {
            await ack();
            const [clientId, variant] = body.actions[0].value.split("|");
            // Re-trigger color command with override to show all
            await handleColorSubcommandWithLimit(body, respond, variant, parseInt(clientId, 10), "all");
        });
        slackApp.action("show_limited_colors", 
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        async ({ ack, body, respond }) => {
            await ack();
            const [clientId, variant] = body.actions[0].value.split("|");
            // Re-trigger color command with limit of 3
            await handleColorSubcommandWithLimit(body, respond, variant, parseInt(clientId, 10), 3);
        });
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        slackApp.action("upload_all_logos", async ({ ack, body, respond }) => {
            await ack();
            const [clientId, query] = body.actions[0].value.split("|");
            // Re-trigger logo command with override to upload all
            await handleLogoSubcommandWithLimit(body, respond, query, parseInt(clientId, 10), "all");
        });
        slackApp.action("upload_limited_logos", 
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        async ({ ack, body, respond }) => {
            await ack();
            const [clientId, query] = body.actions[0].value.split("|");
            // Re-trigger logo command with limit of 3
            await handleLogoSubcommandWithLimit(body, respond, query, parseInt(clientId, 10), 3);
        });
        slackApp.action("process_all_fonts", 
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        async ({ ack, body, respond }) => {
            await ack();
            const [clientId, variant] = body.actions[0].value.split("|");
            // Re-trigger font command with override to process all
            await handleFontSubcommandWithLimit(body, respond, variant, parseInt(clientId, 10), "all");
        });
        slackApp.action("process_limited_fonts", 
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        async ({ ack, body, respond }) => {
            await ack();
            const [clientId, variant] = body.actions[0].value.split("|");
            // Re-trigger font command with limit of 3
            await handleFontSubcommandWithLimit(body, respond, variant, parseInt(clientId, 10), 3);
        });
        ...
        slackApp.action(/.*/, async ({ ack, body, respond }) => {
            ...
            if (actionId === "show_all_colors" ||
                actionId === "show_limited_colors") {
                const [clientId, variant, limit] = actionValue.split("|");
                await handleColorSubcommandWithLimit(body, respond, variant, parseInt(clientId, 10), limit === "all" ? "all" : parseInt(limit, 10));
            }
            else if (actionId === "show_all_logos" ||
                actionId === "show_limited_logos") {
                const [clientId, query, limit] = actionValue.split("|");
                await handleLogoSubcommandWithLimit(body, respond, query, parseInt(clientId, 10), limit === "all" ? "all" : parseInt(limit, 10));
            }
            else if (actionId === "process_all_fonts" ||
                actionId === "process_limited_fonts") {
                const [clientId, variant, limit] = actionValue.split("|");
                await handleFontSubcommandWithLimit(body, respond, variant, parseInt(clientId, 10), limit === "all" ? "all" : parseInt(limit, 10));
            }
        });
        return slackApp;
    }
...
export function registerSlackRoutes(app) {
    ...
    app.post("/api/slack/map-user-test", async (req, res) => {
        ...
        try {
            ...
            const [existingMapping] = await db
                ...
                .where(and(eq(slackUserMappings.slackUserId, slackUserId), eq(slackUserMappings.slackTeamId, slackTeamId)));
            if (existingMapping) {
    ...
    app.post("/api/slack/map-user", async (req, res) => {
        try {
    ...
    app.get("/api/clients/:clientId/slack/mappings", validateClientId, async (req, res) => {
        try {
            ...
            const mappings = await db
                .select()
                .from(slackUserMappings)
                .where(and(eq(slackUserMappings.clientId, clientId), eq(slackUserMappings.isActive, true)));
            res.json(mappings);
        }
        catch (error) {
    ...
    app.get("/api/slack/user-status", async (req, res) => {
        try {
            ...
            const [workspace] = await db
                ...
                .where(and(eq(slackWorkspaces.isActive, true), 
