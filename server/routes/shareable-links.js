...
import { createShareableLink, deactivateShareableLink, getAssetByShareableLink, getAssetShareableLinks, } from "../services/shareable-links";
export function registerShareableLinksRoutes(app) {
    // Create shareable link
    app.post("/api/clients/:clientId/assets/:assetId/share", validateClientId, async (req, res) => {
        try {
    ...
    app.get("/api/clients/:clientId/assets/:assetId/share", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
    ...
    app.delete("/api/clients/:clientId/assets/:assetId/share/:linkId", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
