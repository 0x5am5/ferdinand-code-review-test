...
import { bulkUpdateAssetSharing, getAssetSharing, updateAssetSharing, } from "../services/asset-sharing";
export function registerAssetSharingRoutes(app) {
    // Update sharing settings for a single asset
    app.patch("/api/clients/:clientId/assets/:assetId/visibility", validateClientId, async (req, res) => {
        try {
    ...
    app.patch("/api/clients/:clientId/assets/bulk-visibility", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            const { assetIds, visibility } = req.body;
    ...
    app.get("/api/clients/:clientId/assets/:assetId/visibility", validateClientId, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            const assetId = parseInt(req.params.assetId, 10);
            const sharing = await getAssetSharing(req.session.userId, assetId, req.clientId);
            res.json(sharing);
        }
        catch (error) {
