...
import { EmailServiceError, ERROR_MESSAGES, ErrorResponse, } from "../utils/errorResponse";
...
async function getClientLogoUrl(clientId) {
    try {
        ...
        const logoAsset = findLogoByType(["favicon", "square", "horizontal", "main"]) ||
            logoAssets[0];
...
export function registerInvitationRoutes(app) {
    ...
    app.get("/api/invitations", async (req, res) => {
        try {
            ...
            const enhancedInvitations = await Promise.all(pendingInvitations.map(async (invitation) => {
                let clientData;
                if (invitation.clientIds && invitation.clientIds.length > 0) {
                    try {
                        ...
                        console.error("Error fetching client data for invitation:", err instanceof Error ? err.message : "Unknown error");
                    }
                }
    ...
    app.post("/api/invitations", csrfProtection, invitationRateLimit, async (req, res) => {
        try {
            ...
            if (!parsed.success) {
                return ErrorResponse.validationError(res, ERROR_MESSAGES.VALIDATION_ERROR, parsed.error.errors);
            }
            // Check if a user with this email already exists
            const existingUser = await storage.getUserByEmail(parsed.data.email);
            if (existingUser) {
                return ErrorResponse.conflict(res, ERROR_MESSAGES.EMAIL_EXISTS, "EMAIL_EXISTS");
            }
            ...
            if (pendingInvitation) {
                return ErrorResponse.conflict(res, ERROR_MESSAGES.INVITATION_EXISTS, "INVITATION_EXISTS", { invitationId: pendingInvitation.id });
            }
            ...
            if (req.body.clientIds &&
                ...
                try {
                    ...
                    console.error("Error fetching client data for invitation email:", err instanceof Error ? err.message : "Unknown error");
            ...
            try {
                ...
                console.error("Failed to send invitation email:", emailError instanceof Error ? emailError.message : "Unknown error");
                // If it's a structured EmailServiceError, return it to the frontend
                if (emailError instanceof EmailServiceError) {
                    return ErrorResponse.badRequest(res, emailError.message, emailError.code, emailError.details);
                }
                // For unexpected email errors, return a generic email error
                return ErrorResponse.badRequest(res, ERROR_MESSAGES.EMAIL_SERVICE_FAILED, "EMAIL_SERVICE_FAILED");
            }
    ...
    app.get("/api/invitations/:token", async (req, res) => {
        try {
            ...
            console.error("Error fetching invitation:", error instanceof Error ? error.message : "Unknown error");
    ...
    app.get("/api/invitations/:token/client", async (req, res) => {
        try {
            ...
            console.error("Error fetching client for invitation:", error instanceof Error ? error.message : "Unknown error");
    ...
    app.post("/api/invitations/:id/use", async (req, res) => {
        try {
            ...
            console.error("Error updating invitation:", error instanceof Error ? error.message : "Unknown error");
    ...
    app.delete("/api/invitations/:id", async (req, res) => {
        try {
            ...
            console.error("Error deleting invitation:", error instanceof Error ? error.message : "Unknown error");
    ...
    app.post("/api/invitations/:id/resend", async (req, res) => {
        try {
            ...
            try {
                ...
                console.error("Failed to resend invitation email:", emailError instanceof Error ? emailError.message : "Unknown error");
                // If it's a structured EmailServiceError, return it to the frontend
                if (emailError instanceof EmailServiceError) {
                    return ErrorResponse.badRequest(res, emailError.message, emailError.code, emailError.details);
                }
            ...
            console.error("Error resending invitation:", error instanceof Error ? error.message : "Unknown error");
    ...
    app.get("/api/clients/:clientId/invitations", async (req, res) => {
        try {
            ...
            const clientInvitations = allPendingInvitations.filter((invitation) => invitation.clientIds?.includes(clientId));
            // Enhance invitations with client data
            const enhancedInvitations = await Promise.all(clientInvitations.map(async (invitation) => {
                let clientData;
                if (invitation.clientIds && invitation.clientIds.length > 0) {
                    try {
                        const client = await storage.getClient(invitation.clientIds[0]);
                        ...
                        console.error("Error fetching client data for invitation:", clientError instanceof Error
                            ? clientError.message
                            : "Unknown error");
                    }
                }
            ...
            console.error("Error fetching client invitations:", error instanceof Error ? error.message : "Unknown error");
            res.status(500).json({ message: "Error fetching invitations" });
        }
    });
}
