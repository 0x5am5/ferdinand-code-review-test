...
export function registerDesignSystemRoutes(app: Express) {
    ...
    async (req, res) => {
      try {
        ...
        if (raw_tokens) {
          ...
          if (raw_tokens.colors) {
            ...
            if (raw_tokens.colors.brand) {
              ...
              Object.entries(raw_tokens.colors.brand).forEach(
                ([key, value]) => {
        ...
        if (!theme && !typography && !colors && !raw_tokens) {
          return res
            .status(400)
            .json({ message: "Invalid design system data" });
        }
        ...
        try {
          ...
          console.error(
            "Error reading existing theme.json:",
            error instanceof Error ? error.message : "Unknown error"
          );
        }
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        ...
        if (
          !typographySettings ||
          Object.keys(typographySettings).length === 0
        ) {
          return res
            .status(400)
        ...
        console.error(
          "Error updating typography settings:",
          error instanceof Error ? error.message : "Unknown error"
        );
  ...
  app.get(
    "/api/design-system/export/css/:clientId",
    requireMinimumRole(UserRole.GUEST),
    async (req, res) => {
      try {
...
`;
  ...
  app.get(
    "/api/design-system/export/scss/:clientId",
    requireMinimumRole(UserRole.GUEST),
    async (req, res) => {
      try {
        const { clientId } = req.params;
...
`;
  ...
  app.get(
    "/api/design-system/export/tailwind/:clientId",
    requireMinimumRole(UserRole.GUEST),
    async (req, res) => {
      try {
        const { clientId } = req.params;
        ...
        const config = {
          ...
          plugins: [
            ...
            ({
              ...
              addComponents: (obj: unknown) => void;
              theme: (selector: string) => void;
            }) => {
              addComponents({
                ".btn": {
...
`;
        ...
        console.error(
          "Error exporting Tailwind config:",
          error instanceof Error ? error.message : "Unknown error"
        );
        res.status(500).json({
