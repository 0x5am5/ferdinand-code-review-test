import { insertSlackWorkspaceSchema, slackUserMappings, slackWorkspaces, } from "@shared/schema";
 ...
 */
export function registerSlackOAuthRoutes(app) {
    // Initiate OAuth flow - redirect to Slack (admin only)
    app.get("/api/clients/:clientId/slack/oauth/install", validateClientId, requireAdminRole, async (req, res) => {
        try {
...
/ferdinand help           - Show help
    ...
    app.get("/api/clients/:clientId/slack/workspaces", validateClientId, async (req, res) => {
        try {
            const clientId = req.clientId;
            if (!clientId) {
                return res.status(400).json({ message: "Client ID is required" });
    ...
    app.post("/api/clients/:clientId/slack/workspaces/:workspaceId/reactivate", validateClientId, requireAdminRole, async (req, res) => {
        try {
            const clientId = req.clientId;
            ...
            const [workspace] = await db
                .select()
                .from(slackWorkspaces)
                .where(and(eq(slackWorkspaces.id, workspaceId), eq(slackWorkspaces.clientId, clientId)));
            if (!workspace) {
    ...
    app.delete("/api/clients/:clientId/slack/workspaces/:workspaceId/delete", validateClientId, requireAdminRole, async (req, res) => {
        try {
            const clientId = req.clientId;
            const workspaceId = parseInt(req.params.workspaceId, 10);
            if (!clientId || !workspaceId) {
            ...
            await db
                ...
                .where(and(eq(slackUserMappings.slackTeamId, workspace.slackTeamId), eq(slackUserMappings.clientId, clientId)));
    ...
    app.delete("/api/clients/:clientId/slack/workspaces/:workspaceId", validateClientId, async (req, res) => {
        try {
            const clientId = req.clientId;
            const workspaceId = parseInt(req.params.workspaceId, 10);
            if (!clientId || !workspaceId) {
