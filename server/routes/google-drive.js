...
import { db } from "../db";
import { googleAuthMiddleware, handleGoogleCallback, } from "../middlewares/google-drive-auth";
import { driveImportRateLimit, driveListingRateLimit, } from "../middlewares/rate-limit";
import { createDriveClient, importDriveFile, listDriveFiles, validateFileForImport, } from "../services/google-drive";
...
export function registerGoogleDriveRoutes(app) {
    ...
    app.get("/api/google-drive/status", async (req, res) => {
        try {
            ...
            const [connection] = await db
                ...
                .where(eq((await import("@shared/schema")).googleDriveConnections.userId, req.session.userId));
            if (!connection) {
            ...
            const [user] = await db
                ...
                .where(eq((await import("@shared/schema")).users.id, connection.userId));
            res.json({
                id: connection.id,
    ...
    app.get("/api/google-drive/token", async (req, res) => {
        try {
            ...
            if (isTokenExpired(connection.tokenExpiresAt)) {
                // Refresh the token
                const { refreshUserTokens } = await import("../middlewares/google-drive-auth");
                const refreshedClient = await refreshUserTokens(req.session.userId.toString());
    ...
    app.get("/api/drive/files", driveListingRateLimit, googleAuthMiddleware, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
    ...
    app.post("/api/google-drive/import", driveImportRateLimit, googleAuthMiddleware, async (req, res) => {
        try {
            if (!req.session.userId) {
                return res.status(401).json({ message: "Not authenticated" });
            }
            ...
            if (user.role !== (await import("@shared/schema")).UserRole.SUPER_ADMIN) {
                const [userClient] = await db
                    .select()
                    .from((await import("@shared/schema")).userClients)
                    .where(and(eq((await import("@shared/schema")).userClients.userId, req.session.userId), eq((await import("@shared/schema")).userClients.clientId, clientId)));
                if (!userClient) {
                    return res
                        .status(403)
    ...
    app.delete("/api/google-drive/disconnect", async (req, res) => {
        try {
            ...
            const [connection] = await db
                .select()
                .from((await import("@shared/schema")).googleDriveConnections)
                .where(eq((await import("@shared/schema")).googleDriveConnections.userId, userIdNum));
            if (!connection) {
                return res
                    .status(404)
            ...
            if (tokenToRevoke) {
                try {
                    ...
                    if (!response.ok) {
                        console.warn("Failed to revoke token with Google:", response.status);
                    }
                }
                catch (error) {
