import { insertClientSchema, UserRole, updateClientOrderSchema, } from "@shared/schema";
...
export function registerClientRoutes(app) {
    ...
    app.patch("/api/clients/order", csrfProtection, async (req, res) => {
        try {
            ...
            await Promise.all(clientOrders.map(({ id, displayOrder }) => storage.updateClient(id, { displayOrder })));
            res.json({ message: "Client order updated successfully" });
        }
        catch (error) {
            console.error("Error updating client order:", error instanceof Error ? error.message : "Unknown error");
