...
export async function findMissingDriveLinks(): Promise<ReferenceAssetFix[]> {
  ...
  return missingAssets as ReferenceAssetFix[];
}

export async function fixMissingDriveLink(
  assetId: number,
  driveFileId: string,
  driveWebLink: string
): Promise<boolean> {
  try {
    console.log(
      `Updating asset ${assetId} with driveWebLink: ${driveWebLink}`
    );
...
export function constructDriveWebLink(fileId: string): string {
  ...
  try {
    ...
    for (const asset of missingAssets) {
      if (asset.driveFileId) {
        ...
        const success = await fixMissingDriveLink(
          asset.id,
          asset.driveFileId,
          webLink
        );
        ...
        if (success) {
          fixed++;
          console.log(
            `✓ Fixed asset ${asset.id} (${asset.originalFileName}) → ${webLink}`
          );
        } else {
          console.log(
            `✗ Failed to fix asset ${asset.id} (${asset.originalFileName})`
          );
        }
      } else {
        console.log(
          `⚠ Skipped asset ${asset.id} (${asset.originalFileName}) - no driveFileId`
        );
      }
    }
