...
import { db } from "../db";
import { checkDriveFilePermission, } from "../services/drive-file-permissions";
/**
 ...
 * ```
 */
export function requireDrivePermission(action) {
    return async (req, res, next) => {
        try {
            ...
            const permissionCheck = checkDriveFilePermission(req.session.userId, user.role, action, {
 ...
 */
export async function validateAssetClientAccess(req, res, next) {
    try {
        if (!req.session?.userId) {
            res.status(401).json({ message: "Authentication required" });
            return;
        ...
        const [userClient] = await db
            .select()
            .from(userClients)
            .where(and(eq(userClients.userId, req.session.userId), eq(userClients.clientId, clientId)));
        if (!userClient) {
            res.status(403).json({
 ...
 * ```
 */
export async function checkAssetPermission(userId, assetId, action) {
    try {
        // Fetch the asset
        const [asset] = await db
            .select()
        ...
        // Check permissions (without Drive metadata for now)
        const permissionCheck = checkDriveFilePermission(userId, user.role, action, {
            uploadedBy: asset.uploadedBy,
            visibility: asset.visibility,
            isGoogleDrive: true, // Assume Drive file for permission checks
            driveOwner: asset.uploadedBy, // Use uploader as drive owner
