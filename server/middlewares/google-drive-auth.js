...
import { db } from "../db";
import { decryptTokens, encryptTokens, isTokenExpired, } from "../utils/encryption";
...
export async function refreshUserTokens(userId) {
    try {
        ...
        const { encryptedAccessToken, encryptedRefreshToken, expiresAt } = encryptTokens({
...
export const googleAuthMiddleware = async (req, res, next) => {
    try {
        ...
        if (isTokenExpired(connection.tokenExpiresAt)) {
            console.log(`Token expired for user ${req.session.userId}, refreshing...`);
            try {
                ...
                return res.status(401).json({
                    message: "Google Drive authentication expired. Please re-authenticate.",
                    authUrl,
                    requiresReauth: true,
                });
            }
...
export const handleGoogleCallback = async (req, res) => {
    try {
        ...
        const redirectUrl = stateValue && stateValue !== "undefined" && stateValue !== "dashboard"
