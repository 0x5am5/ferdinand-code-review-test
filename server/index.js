...
async function startServer(retries = 3) {
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            const { ensureDefaultCategories } = await import("./services/default-categories");
...
startServer().catch((error) => {
    console.error("Fatal server error:", error instanceof Error ? error.message : "Unknown error");
    process.exit(1);
});
