...
export class EmailService {
    ...
    constructor() {
        ...
        if (this.usePostmark && apiToken) {
            ...
            console.log("Postmark API token detected. Using Postmark for email delivery.");
        }
        else {
            console.log("No Postmark API token found. Emails will be saved to files instead of being sent.");
        }
    ...
    async sendEmail(options) {
      ...
      `;
            ...
            if (this.usePostmark && this.postmarkClient) {
                try {
                    const fromEmail = process.env.POSTMARK_FROM_EMAIL || "hello@grnbull.com";
                    console.log(`[EMAIL] Using sender email: ${fromEmail}`);
                    console.log(`[EMAIL] Sending via Postmark to: ${options.to}, from: ${fromEmail}`);
            ...
            throw new EmailServiceError("Failed to send email due to an unexpected error.", "EMAIL_SERVICE_FAILED", { originalError: error });
        }
    }
    /**
     ...
     */
    async sendInvitationEmail({ to, inviteLink, clientName = "our platform", expiration = "7 days", logoUrl, }) {
     ...
     */
    async resendInvitationEmail({ to, inviteLink, clientName = "our platform", expiration = "7 days", logoUrl, }) {
     ...
     */
    async sendPasswordResetEmail({ to, resetLink, clientName = "our platform", expiration = "24 hours", }) {
    ...
    `;
        ...
        try {
            ...
            throw new EmailServiceError("Failed to send password reset email.", "EMAIL_SERVICE_FAILED", { originalError: error });
        }
    }
}
