...
interface UpdateSharingOptions {
  userId: number;
  clientId: number;
  assetId: number;
...
interface BulkUpdateSharingOptions {
  userId: number;
  clientId: number;
  assetIds: number[];
  visibility: AssetVisibility;
 ...
 */
export const updateAssetSharing = async (options: UpdateSharingOptions) => {
 ...
 */
export const bulkUpdateAssetSharing = async (
  options: BulkUpdateSharingOptions
) => {
  ...
  const permissionChecks = await Promise.all(
    assetIds.map((assetId) =>
      checkAssetPermission(userId, assetId, clientId, "write")
    )
  );
 ...
 */
export const getAssetSharing = async (
  userId: number,
  assetId: number,
  clientId: number
) => {
