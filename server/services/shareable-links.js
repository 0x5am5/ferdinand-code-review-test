 ...
 */
export const createShareableLink = async (options) => {
    try {
 ...
 */
export const getAssetShareableLinks = async (assetId) => {
    try {
        const now = new Date();
        ...
        const links = await db
            ...
            .where(and(eq(assetPublicLinks.assetId, assetId), isNull(assetPublicLinks.deletedAt), or(isNull(assetPublicLinks.expiresAt), gt(assetPublicLinks.expiresAt, now))));
 ...
 */
export const deactivateShareableLink = async (linkId) => {
    try {
        await db
 ...
 */
export const getAssetByShareableLink = async (token) => {
    try {
        const now = new Date();
        ...
        const [link] = await db
            .select()
            .from(assetPublicLinks)
            .where(and(eq(assetPublicLinks.token, token), isNull(assetPublicLinks.deletedAt), or(isNull(assetPublicLinks.expiresAt), gt(assetPublicLinks.expiresAt, now))));
        if (!link) {
            return null;
        }
        // Get the asset
