...
import { ensureDefaultCategories, getCategoriesForClient, } from "./default-categories";
/**
 * Log audit events for Google Drive operations
 */
async function _logAuditEvent({ userId, clientId, action, fileId, fileName, success, error, metadata, }) {
    try {
...
export const createDriveClient = (auth) => {
    return google.drive({ version: "v3", auth });
};
export const listDriveFiles = async (driveClient, folderId) => {
    try {
        const query = folderId ? `'${folderId}' in parents` : "root in parents";
        const response = await driveClient.files.list({
            q: query,
            fields: "files(id, name, mimeType, size, modifiedTime, webViewLink, webContentLink, thumbnailLink, owners(displayName, emailAddress))",
            pageSize: 1000,
        });
...
export const getFileMetadata = async (driveClient, fileId) => {
    try {
        const response = await driveClient.files.get({
            fileId,
            fields: "id, name, mimeType, size, modifiedTime, webViewLink, webContentLink, thumbnailLink, owners(displayName, emailAddress)",
        });
...
export const importDriveFile = async ({ userId, userRole, clientId, driveFile, visibility, driveClient, }) => {
    try {
        console.log(`Starting import of Drive file: ${driveFile.name} (${driveFile.id})`);
        // Log import start
        ...
        if (isReferenceAsset) {
            console.log(`Importing Google Workspace file as reference asset: ${driveFile.name}`);
            // Update permissions to "anyone with the link can view"
            const permissionResult = await updateFilePermissions(driveClient, driveFile.id);
            if (!permissionResult.success) {
                throw new Error(`Failed to update file permissions: ${permissionResult.error}`);
            }
            ...
            try {
                ...
                categoryId = autoSelectCategory(driveFile.name, driveFile.mimeType, categories);
                if (categoryId) {
                    const categoryName = determineAssetCategory(driveFile.name, driveFile.mimeType);
                    console.log(`Auto-categorized "${driveFile.name}" as "${categoryName}" (ID: ${categoryId})`);
                }
                else {
                    console.log(`No suitable category found for "${driveFile.name}" (${driveFile.mimeType})`);
                }
            }
            catch (categoryError) {
            ...
            if (categoryId) {
                try {
                    ...
                    console.log(`Assigned reference asset ${asset.id} to category ${categoryId}`);
                }
                catch (assignmentError) {
                    console.warn("Failed to assign category to reference asset:", assignmentError);
                    // Continue even if category assignment fails
                }
            }
            console.log(`Google Workspace file imported as reference: ${driveFile.name} (ID: ${asset.id}) by user ${userId} (${userRole})${categoryId ? `, category: ${categoryId}` : ""}`);
            return asset;
        }
        ...
        console.log(`Downloaded ${fileBuffer.length} bytes for file: ${driveFile.name}`);
        ...
        console.log(`Drive file downloaded and imported: ${driveFile.name} -> ${asset.fileName} (ID: ${asset.id}) by user ${userId} (${userRole})${categoryId ? `, category: ${categoryId}` : ""}`);
        return asset;
    }
    catch (error) {
        console.error("Error importing Drive file:", error);
        throw new Error(`Failed to import Drive file: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
};
// Maximum file size for download (100MB)
export const MAX_FILE_SIZE = 100 * 1024 * 1024;
export const validateFileForImport = (driveFile) => {
    // Check file size
 ...
 */
export const updateFilePermissions = async (driveClient, fileId, userId, clientId, fileName) => {
    try {
        ...
        if (response.data) {
            console.log(`Successfully updated permissions for file ${fileId} to anyone with link can view`);
            ...
            console.error(`Failed to update permissions for file ${fileId}:`, response.statusText);
 ...
 */
export const isGoogleWorkspaceFile = (driveFile) => {
